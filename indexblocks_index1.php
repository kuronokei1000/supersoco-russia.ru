...
//debug($arMainPageOrder);
//debug($arTheme);
//debug($bigBannersIndexClass);
if ($arMainPageOrder && is_array($arMainPageOrder)) {
    ...
    foreach ($arMainPageOrder as $key => $optionCode) {
        ...
        $indexBlockClasses = CLite::getIndexBlockClasses($currentBlockOptions);
        ...
        $ajaxPath = $bAjaxBlock ? str_replace(
            '//',
            '/',
            SITE_DIR . $currentBlockOptions['AJAX']['FILE_PATH'] . '/' . $subtype . '/' . $strTemplateName . '.php'
        ) : '';

        if ($bShowBlock) {
            ?>
            <div class="drag-block container <?=$optionCode?> <?=$bBlockIndexClass?>" data-class="<?=$subtype?>_drag"
                 data-order="<?=++$key?>">
