var FilterTagItemDetailPage = function (container, settings)
{
...
FilterTagItemDetailPage.prototype = { 
  ...
  initForms: function ()
  {
    ...
    form.onSuccess = function (data, status, form)
    {
      ...
      let elementName = data.fields.NAME
        ? data.fields.NAME
        : BX.message('SMARTSEO_DEFAULT_TAB_NAME').replace(/#ID#/gi, data.fields.FILTER_TAG_ID);
  ...
  _validateParams: function ()
  {
    if (!this.settings.selectors) {
      console.log('FilterTagItemDetailPage: Array selectors expected')
      return false;
    }

    if (!this.settings.selectors.hasOwnProperty('FORM')) {
      console.log('FilterTagItemDetailPage: Selector FORM expected')
      return false;
    }
    
    if (!this.settings.selectors.hasOwnProperty('PARENT_GRID_ID')) {
      console.log('FilterTagItemDetailPage: Selector PARENT_GRID_ID expected')
      return false;
    }

    if (!this.settings.selectors.hasOwnProperty('PARENT_TAB_CONTROL')) {
      console.log('FilterTagItemDetailPage: Selector PARENT_TAB_CONTROL expected')
      return false;
    }

    if (!AsproUI.DynamicTabsManager.hasInstanceById(this.settings.selectors.PARENT_TAB_CONTROL)) {
      console.log('DynamicTabs "tabs_tag-items" instance not found');
      return false;
    }
