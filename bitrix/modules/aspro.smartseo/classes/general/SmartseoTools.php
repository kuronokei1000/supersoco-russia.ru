...
class SmartseoTools{
	static $moduleClass = 'Aspro\Smartseo\General\Smartseo';

	static function ___1595018847(){
		ob_start();
	...
	static function ___1596018847($data, $signKey = ''){
	...
	static function ___1598018847($hash, $signKey = ''){
		$data = false;
	...
	static function ___1590018847($arData){
		if(is_array($arData)){
			$arResult = array();
	...
	static function ___1593018847($arData){
		if(is_array($arData)){
			$arResult = array();
			foreach($arData as $key => $value){
