...
class phpMorphy_GramTab_GramInfo_Russian extends phpMorphy_GramTab_GramInfo {
    ...
    private function processPos() {
        if($this->pos == 'Р“') {
            if(in_array('РїСЂС‡', $this->grammems)) {
                $this->pos = 'РџР РР§РђРЎРўРР•';
            } elseif(in_array('РґРїСЂ', $this->grammems)) {
                $this->pos = 'Р”Р•Р•РџР РР§РђРЎРўРР•';
            } elseif(in_array('РёРЅС„', $this->grammems)) {
                $this->pos = 'РРќР¤РРќРРўРР’';
            }
        }
    }
