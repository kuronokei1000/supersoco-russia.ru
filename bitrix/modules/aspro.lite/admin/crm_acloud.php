...
if($RIGHT >= "R")
{
	...
	foreach($arSites as $key => $arSite)
	{
		//if(!$arTabsForView || in_array($arSite['ID'], $arTabsForView)){
			...
			$arTabs[] = array(
				...
				"ITEMS" => array(
					"CONFIG" => array(
						"TITLE" => Loc::getMessage("ASPRO_LITE_MODULE_CONFIG_ACLOUD"),
						"ITEMS" => array(
							"DOMAIN_ACLOUD" => array(
								"TYPE" => "text",
								"VALUE" => Option::get($moduleID, "DOMAIN_ACLOUD", "", $optionsSiteID),
							),
							"TOKEN_ACLOUD" => array(
								"TYPE" => "text",
								"VALUE" => Option::get($moduleID, "TOKEN_ACLOUD", "", $optionsSiteID),
								"HINT" => Loc::getMessage("ASPRO_LITE_MODULE_TOKEN_HINT")
							),
						)
					),
					"LINK" => array(
						"TITLE" => Loc::getMessage("ASPRO_LITE_MODULE_LINK_ACLOUD"),
						"ITEMS" => array(
							"ACTIVE_LINK_ACLOUD" => array(
								"TYPE" => "hidden",
								"VALUE" => Option::get($moduleID, "ACTIVE_LINK_ACLOUD", "", $optionsSiteID),
							),
							"ACTIVE_ACLOUD" => array(
								"TYPE" => "checkbox",
								"VALUE" => Option::get($moduleID, "ACTIVE_ACLOUD", "N", $optionsSiteID),
							),
							"AUTOMATE_SEND_ACLOUD" => array(
								"TYPE" => "checkbox",
								"VALUE" => Option::get($moduleID, "AUTOMATE_SEND_ACLOUD", "Y", $optionsSiteID),
								"HINT" => Loc::getMessage("ASPRO_LITE_MODULE_AUTOMATE_SEND_ACLOUD_HINT")
							),
							"USE_LOG_ACLOUD" => array(
								"TYPE" => "checkbox",
								"VALUE" => Option::get($moduleID, "USE_LOG_ACLOUD", "N", $optionsSiteID),
								"HINT" => Loc::getMessage("ASPRO_LITE_MODULE_USE_LOG_ACLOUD_HINT")
							),
							"LEAD_NAME_ACLOUD_TITLE" => array(
								"TYPE" => "text",
								"VALUE" => Option::get($moduleID, "LEAD_NAME_ACLOUD_TITLE", Loc::getMessage("ASPRO_LITE_MODULE_LEAD_NAME_ACLOUD"), $optionsSiteID),
							),
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$Auth.$SendCrm) && $RIGHT >= "W" && check_bitrix_sessid())
	{
		...
		if($Auth)
		{
			if($_POST["DOMAIN"] && $_POST["TOKEN"])
			{
				if(count(explode('.', $_POST["DOMAIN"])) > 2)
					$url = str_replace("#DOMAIN#", $_POST["DOMAIN"], \Aspro\Functions\CAsproLiteCRM::ACLOUD_PATH);
				else
					$url = str_replace("#DOMAIN#", $_POST["DOMAIN"], \Aspro\Functions\CAsproLiteCRM::ACLOUD_SHORT_PATH);

				$arPostFields = array(
					"api_key" => $_POST["TOKEN"]
				);
				echo \Aspro\Functions\CAsproLiteCRM::query($url, "/api/v1/module/crm/lead/list", $arPostFields, array());
			}
			else
			{
				$arError = array(
		...
		elseif($SendCrm)
		{
			...
			{
				echo \Aspro\Functions\CAsproLite::sendLeadCrmFromForm($_POST["FORM_ID"], $_POST["RESULT_ID"], 'ACLOUD', $_POST["SITE_ID"], false, false);
			}
			else
			{
				$arError = array(
		...
		else
		{
			foreach($arTabs as $key => $arTab)
			{
				...
				$domain = Option::get($moduleID, "DOMAIN_ACLOUD", "", $optionsSiteID);
				$token = Option::get($moduleID, "TOKEN_ACLOUD", "", $optionsSiteID);
				$val = "";
				if($domain && $token)
				{
					if(count(explode('.', $domain)) > 2)
						$url = str_replace("#DOMAIN#", $domain, \Aspro\Functions\CAsproLiteCRM::ACLOUD_PATH);
					else
						$url = str_replace("#DOMAIN#", $domain, \Aspro\Functions\CAsproLiteCRM::ACLOUD_SHORT_PATH);

					$arPostFields = array(
						"api_key" => $token
					);
					$result = json_decode(\Aspro\Functions\CAsproLiteCRM::query($url, "/api/v1/module/crm/lead/list", $arPostFields, array()), true);

					if(isset($result["response"]) && $result["response"])
						$val = "Y";
				}
				Option::set($moduleID, "ACTIVE_LINK_ACLOUD", $val, $optionsSiteID);
				...
				if($_POST['CRM_FIELD_'.$optionsSiteID] && $_POST['CRM_FORM_FIELD_'.$optionsSiteID])
				{
					foreach($_POST['CRM_FIELD_'.$optionsSiteID] as $formID => $arFields)
					{
						...
						Option::set($moduleID, 'ACLOUD_CRM_FIELDS_MATCH_'.$formID, serialize($arPostFields), $optionsSiteID);
					}
				}
			}
		}
		...
		*/?>
		...
		foreach($arTabs as $key => $arTab)
		{
			...
			if($arTab["SITE_ID"])
			{
				...
				<?endif;?>
				<?foreach($arTab["ITEMS"] as $groupCode => $arOptions):?>
					<?if($groupCode == "LINK" && !$arOptions["ITEMS"]["ACTIVE_LINK_ACLOUD"]["VALUE"]):?>
						<tr>
							<td colspan="2" style="width:100%;text-align:center;">
								<div class="adm-info-message"><?=Loc::getMessage("ASPRO_LITE_MODULE_INTEGRATION_ACLOUD");?></div>
							</td>
						</tr>
						<?continue;?>
					<?endif;?>

					<tr class="heading"><td colspan="2"><?=Loc::getMessage("ASPRO_LITE_MODULE_".$groupCode."_ACLOUD");?></td></tr>
										...
										<td colspan="2">
											...
											$aSiteTabs = array(
												array(
													...
													"TAB" => Loc::getMessage("ASPRO_LITE_MODULE_FIELDS_ACLOUD"),
													"TITLE" => Loc::getMessage("ASPRO_LITE_MODULE_ALL_FIELDS_ACLOUD"),
													"ICON" => "settings",
													"PAGE_TYPE" => "site_settings",
												),
												array(
													"DIV" => "edit_forms_result",
													"TAB" => Loc::getMessage("ASPRO_LITE_MODULE_RESULTS_ACLOUD"),
													"TITLE" => Loc::getMessage("ASPRO_LITE_MODULE_ALL_RESULTS_ACLOUD"),
													"ICON" => "settings",
													"PAGE_TYPE" => "site_settings",
												),
											);
											$bIntegrationAcloud = (Option::get($moduleID, 'ACTIVE_LINK_ACLOUD', '', $optionsSiteID) && (Option::get($moduleID, 'ACTIVE_ACLOUD', 'N', $optionsSiteID) == 'Y'));
											?>
											...
											<?$siteTabControl->BeginNextTab();?>
												<table cellpadding="0" cellspacing="0" border="0" width="100%" class="edit-table">
													<?foreach($arOption["VALUES"] as $key => $arForm):?>
														...
														$arValueForm = unserialize(Option::get($moduleID, "ACLOUD_CRM_FIELDS_MATCH_".$key, "", $optionsSiteID), ['allowed_classes' => false]);
														?>
																	...
																	<tbody>
																				...
																				<tr>
																					<td class="adm-detail-content-cell-l" style="width:50%;">
																						<select name="CRM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_crm" style="width:300px;">
																							<?foreach(\Aspro\Functions\CAsproLiteCRM::$arCrmFileds["ACLOUD"] as $key2 => $text):?>
																								<option <?=($key2 == $crm_field_id ? "selected" : "");?> value="<?=$key2?>"><?=Loc::getMessage($key2."_ACLOUD")?></option>
																							<?endforeach;?>
																						</select>
																					</td>
																					<td style="width:50%;">
																						<select name="CRM_FORM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_form" style="width:300px;">
																							...
																							<optgroup label="...">
																								<?foreach(\Aspro\Functions\CAsproLiteCRM::$arCrmFileds["MAIN"] as $key2 => $text):?>
																		...
																		<tr>
																			<td class="adm-detail-content-cell-l" style="width:50%;">
																				<select name="CRM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_crm" style="width:300px;">
																					<?foreach(\Aspro\Functions\CAsproLiteCRM::$arCrmFileds["ACLOUD"] as $key2 => $text):?>
																						<option value="<?=$key2?>"><?=Loc::getMessage($key2."_ACLOUD")?></option>
																					<?endforeach;?>
																				</select>
																			</td>
																			<td style="width:50%;">
											...
											<?$siteTabControl->BeginNextTab();?>
												<table cellpadding="0" cellspacing="0" border="0" width="100%" class="edit-table">
													<?foreach($arOption["VALUES"] as $key => $arForm):?>
														...
														<tr class="form_<?=$key;?>" <?=($key != $value ? "style='display: none;'" : '');?>>
															<td colspan="2">
																<?if(!$arValueForm || !$bIntegrationAcloud):?>
																	<div class="adm-info-message"><?=Loc::getMessage("ASPRO_LITE_MODULE_NO_FORM_FIELD_MATCHING");?></div>
																	...
																	<table class="internal" style="width:100%;">
																		...
																		<tbody>
																			<?foreach($arFormResults as $arFormResult):?>
																				...
																				$bSend = (isset($arStatus["ACLOUD"]) && $arStatus["ACLOUD"]);?>
																				<tr>
																					<td>
				...
				$(document).ready(function(){
					...
					$('input.check_auth').on('click', function(){
						...
						$.ajax({
							type: 'POST',
							dataType: 'json',
							data: {'sessid': $('input[name=sessid]').val(), 'Auth': true, 'DOMAIN': tab.find('input[name^=DOMAIN_ACLOUD]').val(), 'TOKEN': tab.find('input[name^=TOKEN_ACLOUD]').val()},
							success: function(data){
					...
					$('select[name^="WEB_FORM_ACLOUD"]').on('change', function(){
						$('tr[class^="form_"]').hide();
						$('tr.form_'+$(this).val()).css('display','');
					})
					$('select[name^="WEB_FORM_ACLOUD"]').change();
				});
