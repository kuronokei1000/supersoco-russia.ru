...
if($RIGHT >= "R"){
	...
	$GLOBALS['APPLICATION']->SetTitle(Loc::getMessage("ASPRO_LITE_PAGE_TITLE"));
	...
	// $arTabsForView = COption::GetOptionString($moduleID, 'TABS_FOR_VIEW_ASPRO_LITE', '');
	// if($arTabsForView){
	...
	foreach($arSites as $key => $arSite){
		//if(!$arTabsForView || in_array($arSite['ID'], $arTabsForView)){
			...
			$arTabs[] = array(
				...
				"ITEMS" => array(
					"CONFIG" => array(
						"TITLE" => Loc::getMessage("ASPRO_LITE_MODULE_CONFIG_AMO_CRM"),
						"ITEMS" => array(
							"DOMAIN_AMO_CRM" => array(
								"TYPE" => "text",
								"VALUE" => Option::get($moduleID, "DOMAIN_AMO_CRM", "", $optionsSiteID),
								"HINT" => Loc::getMessage("ASPRO_LITE_MODULE_DOMAIN_HINT")
							),
							"INTEGRATION_NOTE_AMO_CRM" => array(
								"TYPE" => "note",
								"TITLE" => Loc::getMessage(
									"ASPRO_LITE_MODULE_INTEGRATION_NOTE_AMO_CRM",
									array(
					...
					"LINK" => array(
						"TITLE" => Loc::getMessage("ASPRO_LITE_MODULE_LINK_AMO_CRM"),
						"ITEMS" => array(
							...
							"AUTOMATE_SEND_AMO_CRM" => array(
								...
								"HINT" => Loc::getMessage("ASPRO_LITE_MODULE_AUTOMATE_SEND_AMO_CRM_HINT")
							),
							"USE_LOG_AMO_CRM" => array(
								"TYPE" => "checkbox",
								"VALUE" => Option::get($moduleID, "USE_LOG_AMO_CRM", "N", $optionsSiteID),
								"HINT" => Loc::getMessage("ASPRO_LITE_MODULE_USE_LOG_AMO_CRM_HINT")
							),
							"LEAD_NAME_AMO_CRM_TITLE" => array(
								"TYPE" => "text",
								"VALUE" => Option::get($moduleID, "LEAD_NAME_AMO_CRM_TITLE", Loc::getMessage("ASPRO_LITE_MODULE_LEAD_NAME_AMO_CRM"), $optionsSiteID),
							),
							"TAGS_AMO_CRM_TITLE" => array(
								"TYPE" => "text",
								"VALUE" => Option::get($moduleID, "TAGS_AMO_CRM_TITLE", Loc::getMessage("ASPRO_LITE_MODULE_TAGS_AMO_CRM"), $optionsSiteID),
							),
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$action) && $RIGHT >= "W" && check_bitrix_sessid()){
		...
		if($action === 'checkAuth'){
			...
			if(strlen($arConfig['domain'])){
				// check auth
				$arResponse = CAsproLiteCRM::checkOAuth($arOAuth, $arConfig);
				if(
					$arResponse &&
					is_array($arResponse) &&
					!$arResponse['error']
					...
					CAsproLiteCRM::save($arOAuth, $arConfig);
				}
			}
			else{
				$arResponse = array(
		...
		elseif($action === 'sendCrm'){
			...
			if(
				...
				echo \Aspro\Functions\CAsproLite::sendLeadCrmFromForm($formId, $resultId, 'AMO_CRM', $siteId, false, false);
			}
			else{
				$arResponse = array(
					'error' => 'empty fields',
		...
		// send statistics
		// if(\Aspro\Lite\GS::isEnabled()){
		// 	if(\Aspro\Lite\GS::register()){
		// 		\Aspro\Lite\GS::sendData(
		// 			\Aspro\Lite\GS::mkData(array('sites', 'options'))
		// 		);
		// 	}
		// }
	...
	<?if(!count($arTabs)):?>
		<div class="adm-info-message-wrap adm-info-message-red">
			<div class="adm-info-message">
				<div class="adm-info-message-title"><?=Loc::getMessage("ASPRO_LITE_NO_SITE_INSTALLED", array("#SESSION_ID#"=>bitrix_sessid_get()))?></div>
				<div class="adm-info-message-icon"></div>
			</div>
			<?/*
			<a href="aspro.lite_options_tabs.php" id="tabs_settings" target="_blank">
				<span>
	...
	<?else:?>
		...
		*/?>
			...
			if($arTab["SITE_ID"])
			{
				...
				<?if(!$arTab["FORMS"]):?>
					...
					<td colspan="2" style="width:100%;text-align:center;">
							<div class="adm-info-message"><?=Loc::getMessage("ASPRO_LITE_MODULE_NO_FORMS");?></div>
						</td>
					</tr>
					<?continue;?>
				<?endif;?>
				<?foreach($arTab["ITEMS"] as $groupCode => $arOptions):?>
					<?if($groupCode == "LINK"):?>
						<tr>
							<td colspan="2" style="width:100%;text-align:center;">
								<input type="submit" class="check_auth" value="<?=Loc::getMessage("ASPRO_LITE_MODULE_CHECK_AUTH");?>"/>
								<div><span class="response"></span></div>
							</td>
						</tr>
					<?endif;?>

					<tr class="heading"><td colspan="2"><?=Loc::getMessage("ASPRO_LITE_MODULE_".$groupCode."_AMO_CRM");?></td></tr>

					<?if($groupCode == "LINK" && !$bOAuthChecked):?>
						<tr>
							<td colspan="2" style="width:100%;text-align:center;">
								<div class="adm-info-message"><?=Loc::getMessage("ASPRO_LITE_MODULE_INTEGRATION_AMO_CRM");?></div>
							</td>
						</tr>
						<?continue;?>
					<?endif;?>
						...
						<?else:?>
							<tr>
								<td class="adm-detail-content-cell-l" style="width:50%;<?=($arOption["HINT"] ? "vertical-align: top;padding-top: 7px;" : "");?>">
									<?=Loc::getMessage("ASPRO_LITE_MODULE_".$optionCode);?>
								</td>
								<td style="width:50%;">
										...
										<td colspan="2">
											...
											$aSiteTabs = array(
												array(
													...
													"TAB" => Loc::getMessage("ASPRO_LITE_MODULE_FIELDS_AMO_CRM"),
													"TITLE" => Loc::getMessage("ASPRO_LITE_MODULE_ALL_FIELDS_AMO_CRM"),
													"ICON" => "settings",
													"PAGE_TYPE" => "site_settings",
												),
												array(
													"DIV" => "edit_forms_result",
													"TAB" => Loc::getMessage("ASPRO_LITE_MODULE_RESULTS_AMO_CRM"),
													"TITLE" => Loc::getMessage("ASPRO_LITE_MODULE_ALL_RESULTS_AMO_CRM"),
													"ICON" => "settings",
													"PAGE_TYPE" => "site_settings",
												),
											);
											...
											if($bOAuthChecked){
												$url = str_replace('#DOMAIN#', Option::get($moduleID, 'DOMAIN_AMO_CRM', '', $optionsSiteID), CAsproLiteCRM::AMO_CRM_PATH);
												$result_text = CAsproLiteCRM::query($url, "/private/api/v2/json/accounts/current/", array(), $arHeaders, false, false);
															...
															if($codeGroup != "customers")
															{
																foreach($arGroup as $arProp)
																{
																	...
																	if(isset($arProp["enums"]))
																	{
																		...
																		{
																			CAsproLiteCRM::$arCrmFileds["AMO_CRM"][$codeGroup]["PROPS"][$arProp["id"]."_".$keyEnum."_".$codeGroup] = (Loc::getMessage("AMO_CRM_FIELD_".$arProp["code"]."_".$enum) ? Loc::getMessage("AMO_CRM_FIELD_".$arProp["code"]."_".$enum) : $arProp["name"]."_".$enum);
																		}
																	}
																	else
																	{
																		CAsproLiteCRM::$arCrmFileds["AMO_CRM"][$codeGroup]["PROPS"][$arProp["id"]."_".$codeGroup] = $arProp["name"];
																	}
																}
															}
														}
											...
											<?$siteTabControl->BeginNextTab();?>
												<table cellpadding="0" cellspacing="0" border="0" width="100%" class="edit-table">
													<?foreach($arOption["VALUES"] as $key => $arForm):?>
														...
														$arValueForm = unserialize(Option::get($moduleID, "AMO_CRM_FIELDS_MATCH_".$key, "", $optionsSiteID), ['allowed_classes' => false]);
														?>
																				...
																				<tr>
																					<td class="adm-detail-content-cell-l" style="width:50%;">
																						<select name="CRM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_crm" style="width:300px;">
																							...
																							<?foreach(CAsproLiteCRM::$arCrmFileds["AMO_CRM"] as $groupCode => $arGroup):?>
																					...
																					<td style="width:50%;">
																						<select name="CRM_FORM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_form" style="width:300px;">
																							<option value=""><?=Loc::getMessage('FORM_FIELD_CRM_FIELDS_FORM_NAME_NO')?></option>
																							<optgroup label="...">
																								<?foreach(CAsproLiteCRM::$arCrmFileds["MAIN"] as $key2 => $text):?>
														...
														<tr class="form_<?=$key;?>" <?=($key != $value ? "style='display: none;'" : '');?>>
															<td colspan="2">
																<?if(!$arValueForm || !$bOAuthChecked):?>
																	<div class="adm-info-message"><?=Loc::getMessage("LITE_MODULE_NO_FORM_FIELD_MATCHING");?></div>
																	...
																	<table class="internal" style="width:100%;">
																		...
																		<tbody>
																			<?foreach($arFormResults as $arFormResult):?>
																				<?$arStatus = unserialize(Option::get($moduleID, 'CRM_SEND_FORM_'.$arFormResult["ID"], '', $optionsSiteID), ['allowed_classes' => false]);
																...
																<?else:?>
																	<div class="adm-info-message"><?=Loc::getMessage("ASPRO_LITE_MODULE_NO_FORM_RESULTS");?></div>
																<?endif;?>
															</td>
														</tr>
													<?endforeach;?>
		...
		if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults) && check_bitrix_sessid())
		{
			...
			<input <?if($RIGHT < "W") echo "disabled"?> type="submit" name="Apply" class="submit-btn adm-btn-save" value="<?=Loc::getMessage("ASPRO_LITE_MODULE_SAVE_OPTION")?>" title="<?=Loc::getMessage("ASPRO_LITE_MODULE_SAVE_OPTION")?>">
			<input type="submit" name="RestoreDefaults" title="<?=Loc::getMessage("ASPRO_LITE_MODULE_DELETE_OPTION")?>" onclick="confirm('<?=Loc::getMessage("ASPRO_LITE_MODULE_DELETE_OPTION_TITLE")?>')" value="<?=Loc::getMessage("ASPRO_LITE_MODULE_DELETE_OPTION_TEXT")?>">

			<script type="text/javascript">
				BX.message({
					"CRM_SEND": "<?=Loc::getMessage("FORM_RESULT_SEND")?>",
					"ASPRO_LITE_MODULE_LINK_COPY": "<?=Loc::getMessage("ASPRO_LITE_MODULE_LINK_COPY")?>",
					"ASPRO_LITE_MODULE_LINK_COPYED": "<?=Loc::getMessage("ASPRO_LITE_MODULE_LINK_COPYED")?>",
					"ASPRO_LITE_MODULE_LINK_COPY_ERROR": "<?=Loc::getMessage("ASPRO_LITE_MODULE_LINK_COPY_ERROR")?>",
				});
				...
				$(document).ready(function(){
					...
					if($btnCopyLink.length){
						...
						clipboard.on('success', function(e){
						    $btnCopyLink.html('<i class="fa fa-check"></i>' + BX.message('ASPRO_LITE_MODULE_LINK_COPYED'))
							...
							copyLinkTimeout = setTimeout(function(){
								$btnCopyLink.html('<i class="fa fa-clipboard"></i>' + BX.message('ASPRO_LITE_MODULE_LINK_COPY'))
							}, 2000)
						...
						clipboard.on('error', function(e){
						    alert(BX.message('ASPRO_LITE_MODULE_LINK_COPY_ERROR') + $btnCopyLink.attr('data-clipboard-text'))
						});
					}
					...
					$('input.check_auth').on('click', function(e){
						...
						var siteId = tab.find('input[name^=DOMAIN_AMO_CRM]').attr('name').replace(/.+_(.{2})$/, '$1');

						tab.find('.response').empty();
						...
						$.ajax({
							...
							data: {
								...
								action: 'checkAuth',
								siteId: siteId,
								domain: tab.find('input[name^=DOMAIN_AMO_CRM]').val(),
								clientSecret: tab.find('input[name^=CLIENT_SECRET_AMO_CRM]').val(),
								clientId: tab.find('input[name^=CLIENT_ID_AMO_CRM]').val(),
								authCode: tab.find('input[name^=AUTH_CODE_AMO_CRM]').val()
							},
							success: function(data){
								if('response' in data){
									data = data.response;
								...
								if('error' in data){
									tab.find('.response').removeClass('success').addClass('error').text(data.error);
									tab.find('input[name^=AUTH_CODE_AMO_CRM]').prop('readonly', false);
								}
								else{
									tab.find('.response').removeClass('error').addClass('success').text('ok');
									tab.find('input[name^=AUTH_CODE_AMO_CRM]').prop('readonly', true);
								}
							},
							error: function(data){
								window.console&&console.log(data);
								tab.find('.response').removeClass('success').addClass('error').text('error');
							},
							complete: function(){
