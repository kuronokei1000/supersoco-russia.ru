...
if (!class_exists('CLiteRegionality')) {
	class CLiteRegionality
	{
		...
		{
			if(Loader::includeModule(LITE_MODULE_ID))
			{
		...
		public static function getRegionIBlockID()
		{
			...
			{
				if(isset(SolutionCache::$arIBlocks[SITE_ID]['aspro_lite_regionality']['aspro_lite_regions'][0]) && SolutionCache::$arIBlocks[SITE_ID]['aspro_lite_regionality']['aspro_lite_regions'][0])
				{
					$iRegionIBlockID = SolutionCache::$arIBlocks[SITE_ID]['aspro_lite_regionality']['aspro_lite_regions'][0];
				}
				else
				{
					return;
		...
		public static function replaceSeoMarks()
		{
			...
			{
				foreach(self::$arSeoMarks as $mark => $field)
				{
				...
				{
					$bShowSiteName = (\Bitrix\Main\Config\Option::get(LITE_MODULE_ID, "HIDE_SITE_NAME_TITLE", "N") == "N");
		...
		public static function getRegions()
		{
			...
			if($arRegions === NULL)
			{
				...
				if($iRegionIBlockID = self::getRegionIBlockID())
				{
					...
					$cache_id = 'aspro_lite_regions'.$iRegionIBlockID;
					...
					{
						// get all items
						$arMainProps = array('DEFAULT', 'DOMAINS', 'COMPANY_TITLE', 'MAIN_DOMAIN', 'FAVORIT_LOCATION', 'PHONES', 'PRICES_LINK', 'LOCATION_LINK', 'STORES_LINK', 'REGION_NAME_DECLINE_RP', 'REGION_NAME_DECLINE_PP', 'REGION_NAME_DECLINE_TP', 'SORT_REGION_PRICE','OLD_REGION_PRICES', 'ADDRESS', 'EMAIL', 'SHCEDULE');
						...
						foreach(GetModuleEvents(LITE_MODULE_ID, 'OnAsproRegionalityAddSelectFieldsAndProps', true) as $arEvent) // event for add to select in region getlist elements
							ExecuteModuleEventEx($arEvent, array(&$arSelect));

						// $arItems = SolutionCache::CIBLockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => SolutionCache::GetIBlockCacheTag($iRegionIBlockID), 'GROUP' => 'ID')), $arFilter, false, false, $arSelect);
						...
						foreach(GetModuleEvents(LITE_MODULE_ID, 'OnAsproRegionalityGetElements', true) as $arEvent) // event for manipulation with region elements
