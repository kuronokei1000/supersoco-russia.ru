...
if(!defined('LITE_MODULE_ID'))
	define('LITE_MODULE_ID', 'aspro.lite');
...
class CLite {
	...
	const solutionName	= 'lite';
	const templateName	= 'aspro-lite';
	const moduleID		= 'aspro.lite';
	const wizardID		= 'aspro:lite';
	...
	public static function isSaleMode() {
		...
		return $result ?? ($result = (Loader::includeModule('sale') && Option::get(self::moduleID, 'IS_SALE_MODE', 'Y', SITE_ID) === 'Y'));
	}
	...
	public static function checkModuleRight($reqRight = 'R', $bShowError = false, $arExcludeRights = []){
		global $APPLICATION;

		$curRight = $APPLICATION->GetGroupRight(self::moduleID);

		if(
			$curRight < $reqRight ||
			(
				$arExcludeRights &&
				in_array($curRight, $arExcludeRights)
			)
		){
			if($bShowError){
				$APPLICATION->AuthForm(GetMessage('LITE_ACCESS_DENIED'));
			}
	...
	public static function GetSections($arItems, $arParams)
	{
		...
		if (is_array($arItems) && $arItems) {
			...
			if ($arSectionsIDs) {
				...
				$arSections['ALL_SECTIONS'] = CLiteCache::CIBLockSection_GetList(
					array(
						'SORT' => 'ASC',
						'NAME' => 'ASC',
						'CACHE' => $arCacheParams,
				...
				foreach ($arSections['ALL_SECTIONS'] as $key => $arSection) {
					if ($arSection['DEPTH_LEVEL'] > 1) {
						...
						$arParentSection = CLiteCache::CIBLockSection_GetList(
							array(
								'SORT' => 'ASC',
								'NAME' => 'ASC',
								'CACHE' => $arCacheParams,
				...
				if ($bCheckRoot) {
					...
					$arRootSections = CLiteCache::CIBLockSection_GetList(
						array(
							'SORT' => 'ASC',
							'NAME' => 'ASC',
							'CACHE' => $arCacheParams,
	...
	{
		if ($handler = CAsproLite::getCustomFunc(__FUNCTION__)) {
			call_user_func_array($handler, [$arParams, &$aMenuLinksExt, $bMenu]);
			return;
		}

		$catalog_id = \Bitrix\Main\Config\Option::get('aspro.lite', 'CATALOG_IBLOCK_ID', CLiteCache::$arIBlocks[SITE_ID]['aspro_lite_catalog']['aspro_lite_catalog'][0]);
		$bIsCatalog = $arParams['IBLOCK_ID'] == $catalog_id;

		$arParams['CATALOG_IBLOCK_ID'] = $catalog_id;
		$arParams['IS_CATALOG_IBLOCK'] = $bIsCatalog;

		foreach(GetModuleEvents(LITE_MODULE_ID, 'BeforeAsproGetMenuChildsExt', true) as $arEvent) // event for manipulation store quantity block
		...
		$arSectionFilter = array(
			...
			'GLOBAL_ACTIVE' => 'Y',
			'ACTIVE_DATE' => 'Y',
			'<DEPTH_LEVEL' => self::GetFrontParametrValue('MAX_DEPTH_MENU'),
		);
		...
		{
			$arSections = CLiteCache::CIBlockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CLiteCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), $arSectionFilter, false, $arSectionSelect);
			$arSectionsByParentSectionID = CLiteCache::GroupArrayBy($arSections, array('MULTI' => 'Y', 'GROUP' => array('IBLOCK_SECTION_ID')));
		}
		...
		if(!$bIsCatalog) {
			if($arParams['MENU_PARAMS']['MENU_SHOW_ELEMENTS'] == 'Y'){
				...
				$arItems = CLiteCache::CIBlockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CLiteCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), $arElementFilter, false, false, $arElementSelect);
				...
				if($arParams['MENU_PARAMS']['MENU_SHOW_SECTIONS'] == 'Y'){
					$arItemsBySectionID = CLiteCache::GroupArrayBy($arItems, array('MULTI' => 'Y', 'GROUP' => array('IBLOCK_SECTION_ID')));
				}
				/*else{
					$arItemsRoot = CLiteCache::CIBlockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CLiteCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'ACTIVE' => 'Y', 'ACTIVE_DATE' => 'Y', 'SECTION_ID' => 0));
					$arItems = array_merge((array)$arItems, (array)$arItemsRoot);
				}*/
			}
		}

		foreach(GetModuleEvents(LITE_MODULE_ID, 'OnAsproGetMenuChildsExt', true) as $arEvent) // event for manipulation store quantity block
			ExecuteModuleEventEx($arEvent, array($arParams, &$aMenuLinksExt, $bMenu));

		if($arSections) {
			CLite::getSectionChilds(false, $arSections, $arSectionsByParentSectionID, $arItemsBySectionID, $aMenuLinksExt, $bMenu);
		}

		if(!$bIsCatalog) {
			if($arItems && ($arParams['MENU_PARAMS']['MENU_SHOW_SECTIONS'] != 'Y' || !$arSections)){
				foreach($arItems as $arItem){
		...
		foreach(GetModuleEvents(LITE_MODULE_ID, 'AfterAsproGetMenuChildsExt', true) as $arEvent) // event for manipulation store quantity block
			ExecuteModuleEventEx($arEvent, array($arParams, &$aMenuLinksExt, $bMenu));
	}
	...
	public static function replaceMenuChilds(&$arResult, $arParams)
	{
		...
		$menuIblockId = CLiteCache::$arIBlocks[SITE_ID]['aspro_lite_catalog']['aspro_lite_megamenu'][0];
		if($menuIblockId){
			$arMenuSections = CLiteCache::CIblockSection_GetList(
				array(
					'SORT' => 'ASC',
					'NAME' => 'ASC',
					'CACHE' => array(
						'TAG' => CLiteCache::GetIBlockCacheTag($menuIblockId),
	...
	public static function CompareMenuItems($parentMenu, $childMenu)
	{
		...
		$parentMenu = is_array($parentMenu) ? $parentMenu : [];
		...
		if ((is_array($arTheme) && $arTheme) || $bShowBlock) {
			switch($type):
				...
				case 'left_block':
					...
					break;
				case 'footer':
					if (self::IsBasketPage() || self::IsOrderPage()) {
						$APPLICATION->SetAdditionalCSS(SITE_TEMPLATE_PATH.'/css/footer-simple.css');
						$file = $_SERVER['DOCUMENT_ROOT'].SITE_TEMPLATE_PATH.'/page_blocks/'.$type.'_simple_'.$arTheme['FOOTER_SIMPLE']['VALUE'].'.php';
					}
					else {
				...
				case 'header':
					if (self::IsBasketPage() || self::IsOrderPage()) {
						$APPLICATION->SetAdditionalCSS(SITE_TEMPLATE_PATH.'/css/header-simple.css');
						$file = $_SERVER['DOCUMENT_ROOT'].SITE_TEMPLATE_PATH.'/page_blocks/'.$type.'_simple_'.$arTheme['HEADER_SIMPLE']['VALUE'].'.php';
					}
					else {
				...
				default:
					...
					foreach(GetModuleEvents(LITE_MODULE_ID, 'OnAsproShowPageType', true) as $arEvent) // event for manipulation arMainPageOrder
			...
			if($type === 'footer'){
				...
				$checkProperty = CAsproLite::checkProperty($APPLICATION->GetProperty('NAME_BUTTON_DOWNLOAD'), $APPLICATION->GetProperty('URL_BUTTON_DOWNLOAD'));
				if($checkProperty){
	...
	public static function ShowLogo($arParams = array()){
		...
		$arImg = self::unserialize(Option::get(LITE_MODULE_ID, $logoOptionCode, serialize(array()))) ?: ($isWhite ? self::unserialize(Option::get(LITE_MODULE_ID, 'LOGO_IMAGE', serialize(array()))) : array());
	...
	public static function ShowBufferedLogo() {
		...
		$GLOBALS['APPLICATION']->AddBufferContent(['CLite', 'GetBufferedLogo']);
	...
	public static function ShowBufferedFixedLogo() {
		...
		$GLOBALS['APPLICATION']->AddBufferContent(['CLite', 'GetBufferedFixedLogo']);
	...
	public static function ShowBufferedMobileLogo() {
		...
		$GLOBALS['APPLICATION']->AddBufferContent(['CLite', 'GetBufferedMobileLogo']);
		...
		if(!$arValues){
			...
			if(isset(self::$arParametrsList['WEB_FORMS']['OPTIONS']) && defined('ADMIN_SECTION')){
				if(!$arWebForms[$SITE_ID]){
					if($arWebForms[$SITE_ID] = \Aspro\Lite\Property\ListWebForms::getWebForms(array($SITE_ID))){
						// add form`s options
			...
			if (self::isSaleMode()) {
				...
				if(isset(self::$arParametrsList['BASKET']['OPTIONS']['ORDER_VIEW']['DEPENDENT_PARAMS']['BASKET_FILE_DOWNLOAD_TEMPLATE'])){
					ob_start();
					\CBitrixComponent::includeComponentClass('aspro:basket.file.lite');
					ob_get_clean();
					if(class_exists('CAsproBasketFileLite')){
						self::$arParametrsList['BASKET']['OPTIONS']['ORDER_VIEW']['DEPENDENT_PARAMS']['BASKET_FILE_DOWNLOAD_TEMPLATE']['LIST'] = CAsproBasketFileLite::getTemplatesList('/bitrix/templates/aspro-lite');
					}
				}
			} else {
				unset(self::$arParametrsList['BASKET']['OPTIONS']['ORDER_VIEW']['DEPENDENT_PARAMS']['SHOW_SHARE_BASKET']);
				unset(self::$arParametrsList['BASKET']['OPTIONS']['ORDER_VIEW']['DEPENDENT_PARAMS']['BASKET_FILE_DOWNLOAD_TEMPLATE']);
				unset(self::$arParametrsList['BASKET']['OPTIONS']['ORDER_VIEW']['DEPENDENT_PARAMS']['SHOW_DOWNLOAD_BASKET']);
				unset(self::$arParametrsList['BASKET']['OPTIONS']['ORDER_VIEW']['DEPENDENT_PARAMS']['SHOW_DOWNLOAD_BASKET']);
				unset(self::$arParametrsList['BASKET']['OPTIONS']['ORDER_VIEW']['DEPENDENT_PARAMS']['BUYMISSINGGOODS']['LIST']['SUBSCRIBE']);
				unset(self::$arParametrsList['ONECLICKBUY']);
				unset(self::$arParametrsList['CATALOG_PAGE']['OPTIONS']['PRICES_TYPE']);
				unset(self::$arParametrsList['CATALOG_PAGE']['OPTIONS']['STORES']);
				unset(self::$arParametrsList['CATALOG_PAGE']['OPTIONS']['HIDE_NOT_AVAILABLE']);
				unset(self::$arParametrsList['CATALOG_PAGE']['OPTIONS']['HIDE_NOT_AVAILABLE_OFFERS']);
				unset(self::$arParametrsList['CATALOG_PAGE']['OPTIONS']['DISCOUNT_PRICE']);
				unset(self::$arParametrsList['CATALOG_PAGE']['OPTIONS']['CONVERT_CURRENCY']);
				unset(self::$arParametrsList['CATALOG_PAGE']['OPTIONS']['CURRENCY_ID']);
				unset(self::$arParametrsList['CATALOG_PAGE']['OPTIONS']['PRICE_VAT_INCLUDE']);
				unset(self::$arParametrsList['CATALOG_PAGE']['OPTIONS']['SHOP_WINDOW_MODE']);
			}
			...
			if($arTemplate && $arTemplate['PATH']){
				...
				// add custom values for HEADER_SIMPLE
				if(isset(self::$arParametrsList['HEADER']['OPTIONS']['HEADER_SIMPLE']))
					self::Add2OptionCustomPageBlocks(self::$arParametrsList['HEADER']['OPTIONS']['HEADER_SIMPLE'], $arTemplate['PATH'].'/page_blocks/', 'header_simple_custom', 'custom');
				...
				// add custom values for FOOTER_SIMPLE
				if(isset(self::$arParametrsList['FOOTER']['OPTIONS']['FOOTER_SIMPLE']))
					self::Add2OptionCustomPageBlocks(self::$arParametrsList['FOOTER']['OPTIONS']['FOOTER_SIMPLE'], $arTemplate['PATH'].'/page_blocks/', 'footer_simple_custom', 'custom');
						...
						foreach($arBlock['OPTIONS'] as $optionCode => $arOption)
						{
							if($arOption['TYPE'] !== 'note' && $arOption['TYPE'] !== 'includefile'){
								if($arOption['TYPE'] === 'array'){
									...
									\Aspro\Functions\CAsproLiteAdmin::getBackParams($arValues, $arDefaultValues, $arOption, $SITE_ID);
									...
									if(isset($arOption['DEPENDENT_PARAMS']) && $arOption['DEPENDENT_PARAMS']) //get dependent params default value
									{
										foreach($arOption['DEPENDENT_PARAMS'] as $key => $arSubOption)
										{
											...
											if(
												$key === 'USE_1CLICK_GOALS' ||
												$key === 'USE_FASTORDER_GOALS'
											) {
												if (!\Bitrix\Main\Loader::includeModule('catalog')) {
													self::$arParametrsList[$blockCode]['OPTIONS'][$optionCode]['DEPENDENT_PARAMS'][$key]['DISABLED'] = 'Y';
												}
											}

											elseif($key === 'USE_PHONE_AUTH')
											{
												list($bPhoneAuthSupported, $bPhoneAuthShow, $bPhoneAuthRequired, $bPhoneAuthUse) = Aspro\Lite\PhoneAuth::getOptions();
			...
			foreach($arValues as $key => $value)
			{
				if(
					...
					else
					{
						...
						elseif($key === 'LOGO_IMAGE_EMAIL')
							$arValues[$key] = str_replace('//', '/', $SITE_DIR.'/include/logo.png');
					}
					...
					<div class="item js_block <?=$optionType;?> <?=((isset($arOption["WITH_HINT"]) && $arOption["WITH_HINT"] == "Y") ? 'with-hint' : '');?> <?=((isset($arOption["BIG_BLOCK"]) && $arOption["BIG_BLOCK"] == "Y") ? 'big-block' : '');?>" data-class="<?=$optionCode;?>" data-search="<?=$searchClass;?>">
							...
							<?foreach($arOption['LIST'] as $key => $value):?>
								...
								if($arOption['SUB_PARAMS'][$key]):?>
									<div class="parent-wrapper js-sub block_<?=$key.'_'.$optionsSiteID;?>" <?=($optionVal == $key ? "style='display:block;'" : "")?>>
										...
										$arNewOptions = CAsproLite::getCustomBlocks($optionsSiteID);
										...
										if ($arNewOptions) {
											...
											foreach ($arNewOptions as $keyOption => $arNewOption) {
												...
												if (!$arTab['OPTIONS'][$fieldKey]) {
													$arTmpValues = self::unserialize(Option::get(self::moduleID, 'NESTED_OPTIONS_'.$optionCode.'_'.$key, serialize(array()), $optionsSiteID));
						...
						<?if(isset($arOption['DEPENDENT_PARAMS']) && $arOption['DEPENDENT_PARAMS']): //dependent params?>
							<?foreach($arOption['DEPENDENT_PARAMS'] as $key => $arValue):?>
								<?if ($arValue['GROUP_BLOCK']) continue;?>
								<?
								$searchClass = "";
								if($bSearchMode)
								{
			...
			<?if(Loader::IncludeModule('iblock')):?>
				<div colspan="2">
					...
					$arFilter = array(
						...
						"TYPE" => "aspro_lite_form"
					);
					$rsItems = CIBlock::GetList($arSort, $arFilter);
					while($arItem = $rsItems->Fetch()){
						if($arItem["CODE"] != "aspro_lite_example" && $arItem["CODE"] != "aspro_lite_order_page")
						{
		...
		<?elseif($optionType == "note"):?>
			<?if($optionCode == 'GOALS_NOTE')
			{
				...
				if(\Bitrix\Main\Loader::includeModule('form'))
				{
					if($optionsSiteID)
					{
						if($arForms = CLiteCache::CForm_GetList($by = array('by' => 's_id', 'CACHE' => array('TAG' => 'forms')), $order = 'asc', array('SITE' => $optionsSiteID, 'SITE_EXACT_MATCH' => 'Y'), $is_filtered))
						{
			...
			<?endif;?>
							...
							<div>
								<div class="aspro-admin-item">
									<?foreach($arPriority as $i => $priorityCode):?>
										...
										if($priorityCode === 'SMARTSEO'){
											if(!self::isSmartSeoInstalled()){
												$bDisabled = true;
												$subtitle .= ' '.Loc::getMessage('NOT_INSTALLED', array('#MODULE_NAME#' => 'aspro.smartseo'));
											}
										}
										?>
				...
				<?elseif($optionType == "selectbox"):?>
					...
					if(isset($arOption['TYPE_SELECT']))
					{
						...
						{
							static $arIblockProps;
							if (Loader::includeModule('iblock')) {
								if($arIblockProps === null && $arOption['PROPS_SETTING']){
									$arFilter = [];
									...
									while ($arProp = $rsProps->GetNext()) {
										$arIblockProps[$arProp['ID']] = "[{$arProp['ID']}] {$arProp['NAME']} ({$arProp['CODE']})";
									}
								}
							}
						...
						elseif($arOption['TYPE_SELECT'] == 'CURRENCIES')
						{
							...
							if (Loader::includeModule('currency')) {
								if($arCurrencies === null){
									$currencyIterator = \CCurrency::GetList(
										$by='sort',
										$order='asc',
										'ru'
									);
						...
						<select  <?=((isset($arOption['DEPENDENT_PARAMS']) && $arOption['DEPENDENT_PARAMS']) ? "class='depend-check'" : "");?> data-site="<?=$optionsSiteID?>" name="<?=htmlspecialcharsbx($optionCode)."_".$optionsSiteID?>" <?=$optionController?> <?=$optionDisabled?>>
							<?if($bIBlocks)
							{
								foreach($arIBlocks as $key => $arValue) {
									$selected="";
									if(!$optionVal && isset($arOption["DEFAULT_CODE"]) && $arValue["CODE"] === $arOption["DEFAULT_CODE"]){
										$selected="selected";
				...
				<?elseif($optionType == "multiselectbox"):?>
					...
					if(isset($arOption['TYPE_SELECT']))
					{
						...
						elseif($arOption['TYPE_SELECT'] == 'STORES')
						{
							static $arStores;
							if (Loader::includeModule('catalog')) {
								if($arStores === null){
									...
									while ($store = $storeIterator->GetNext())
										$arStores[$store['ID']] = "[".$store['ID']."] ".$store['TITLE'];
								}
							}
							$optionList = $arStores;
							if (!$arStores) {
								$textNote = GetMessage('NEED_ADD_STORES');
							}
						}
						elseif($arOption['TYPE_SELECT'] == 'IBLOCK_PROPS')
						{
					...
					<?else:?>
						<select data-site="<?=$optionsSiteID?>" size="<?=$optionSize?>" <?=$optionController?> <?=$optionDisabled?> multiple name="<?=htmlspecialcharsbx($optionCode)."_".$optionsSiteID?>[]" >
							...
							if(
								...
								if(\CLiteCache::$arIBlocks[$optionsSiteID]){
									foreach(CLiteCache::$arIBlocks[$optionsSiteID] as $iblockTypeId => $arIblockCodes) {
											...
											foreach($arIblockCodes as $iblockCode => $iblocksIDs) {
												if ($iblocksIDs) {
													foreach($iblocksIDs as $iblockId) {
														...
														$selected = is_array($optionVal) && $optionVal && in_array($key, $optionVal) ? 'selected' : '';
														?><option value="<?=$key?>" <?=$selected?>><?='['.CLiteCache::$arIBlocksInfo[$iblockId]["ID"].'] '.htmlspecialcharsbx(CLiteCache::$arIBlocksInfo[$iblockId]["NAME"])?></option><?
													}
												}
											}
										?></optgroup><?
	...
	public static function checkBgImage($siteID){
		...
		if($arBanner === NULL)
		{
			$bgIbockID = (CLiteCache::$arIBlocks[$siteID]['aspro_lite_content']['aspro_lite_bg_images'][0] ? CLiteCache::$arIBlocks[$siteID]['aspro_lite_content']['aspro_lite_bg_images'][0] : CLiteCache::$arIBlocks[$siteID]['aspro_lite_adv']['aspro_lite_bg_images'][0]);
			...
			$arItems = CLiteCache::CIBLockElement_GetList(array('SORT' => 'ASC', 'CACHE' => array('TAG' => $bgIbockID)), $arFilterBanner, false, false, array('ID', 'NAME', 'PREVIEW_PICTURE', 'PROPERTY_URL', 'PROPERTY_FIXED_BANNER', 'PROPERTY_URL_NOT_SHOW'));
			$arBanner = array();
					...
					if (!$bMenu) {
						$arItem = array(
							...
							array(
								...
								'ICON' => $arSection['UF_SECTION_ICON'] ?? $arSection['UF_CATALOG_ICON'] ?? $arSection['UF_ICON'] ?? false,
	...
	public static function SetJSOptions(){
		...
		<?if($arFrontParametrs['YANDEX_ECOMERCE'] == 'Y' || $arFrontParametrs['GOOGLE_ECOMERCE'] == 'Y'):?>window.dataLayer = window.dataLayer || [];<?endif;?>
	...
	public static function SetMeta(){
		...
		if(!strlen(self::$arMetaParams['og:image'])){
			self::$arMetaParams['og:image'] = $arTheme['LOGO_IMAGE_OG']['VALUE'] ?: $arTheme['LOGO_IMAGE']['VALUE'];
		}
	...
	public static function setRegionSeoMarks(){
		...
		if ($arRegion) {
			...
			if ($arTagSeoMarks) {
				CLiteRegionality::addSeoMarks($arTagSeoMarks);
			}
		}
	}
	...
	public static function getSectionInheritedUF($arParams = array()){
		...
		if($arParams){
			...
			if($sectionId && $arSelect){
				...
				$arSection = CLiteCache::CIBlockSection_GetList(
					array(
						'CACHE' => array(
							'MULTI' => 'N',
	...
	public static function isCabinetAvailable() {
		return self::GetFrontParametrValue('CABINET') === 'Y';
	}

	public static function isPersonalSectionAvailable() {
		return self::isCabinetAvailable() && $GLOBALS['USER']->IsAuthorized();
	}

	public static function isPersonalSaleSectionAvailable() {
		return self::isPersonalSectionAvailable() && \Bitrix\Main\Loader::includeModule('sale');
	}
	...
	public static function ShowCabinetLink($arOptions){
		...
		global $APPLICATION, $arTheme, $USER;
		...
		$bTextRight = isset($arOptions['TEXT_RIGHT']) && $arOptions['TEXT_RIGHT'] === "Y";
		...
		<?if($userID):?>
			<?
			$loginText = $arOptions['TEXT_LOGIN'] === 'login' ? ($USER->GetFirstName() ?: $USER->GetLastName() ?: $USER->GetLogin()) : $arOptions['TEXT_LOGIN'];
			$loginText = htmlspecialchars($loginText);
			?>
			<a class="header-cabinet__link fill-theme-hover fill-dark-light-block light-opacity-hover dark_link avt <?=$arOptions['CLASS_LINK']?>" title="<?=$loginText;?>" href="<?=$arTheme['PERSONAL_PAGE_URL']['VALUE']?>">
				<?if($arOptions['SHOW_SVG']):?>
					<?=self::showSpriteIconSvg(SITE_TEMPLATE_PATH.'/images/svg/header_icons.svg#log-in-22-20', 'header__icon banner-light-icon-fill menu-light-icon-fill fill-dark-light-block', ['WIDTH' => 22,'HEIGHT' => 20]);?>
				<?endif;?>
				<?if($arOptions['SHOW_ICON_NAME'] && $loginText):?>
					<span class="header-cabinet__name header__icon-name menu-light-text banner-light-text <?=$bTextRight ? 'header__icon-name--right' : ''?>"><?=$loginText;?></span>
				<?endif;?>
			</a>
		...
		<?else:?>
			...
			<a class="header-cabinet__link fill-theme-hover fill-dark-light-block light-opacity-hover dark_link animate-load <?=$arOptions['CLASS_LINK']?>" data-event="jqm" title="<?=$arOptions['TEXT_NO_LOGIN'] ?: GetMessage('LOGIN'); ?>" data-param-backurl="<?=urlencode($url)?>" data-param-type="auth" data-name="auth" href="<?=$arTheme['PERSONAL_PAGE_URL']['VALUE']?>">
				<?if($arOptions['SHOW_SVG']):?>
					<?=self::showSpriteIconSvg(SITE_TEMPLATE_PATH.'/images/svg/header_icons.svg#log-out-18-18', 'header__icon banner-light-icon-fill menu-light-icon-fill', ['WIDTH' => 18,'HEIGHT' => 18]);?>
				<?endif;?>
				<?if($arOptions['TEXT_NO_LOGIN'] && $arOptions['SHOW_ICON_NAME']):?>
					<span class="header-cabinet__name header__icon-name menu-light-text banner-light-text <?=$bTextRight ? 'header__icon-name--right' : ''?>"><?=$arOptions['TEXT_NO_LOGIN']?></span>
				<?endif;?>
			</a>
		<?endif;?>
		<!-- /noindex -->
	...
	public static function ShowBasketLink($class_link = 'top-btn hover', $class_icon = '', $txt = '', $show_price = false){
		...
		if(
			...
			$arItems = Aspro\Lite\Itemaction\Basket::getItems();
			$count = count($arItems['BASKET']);
			$bEmptyBasket = !$count;
			$title_text = Aspro\Lite\Itemaction\Basket::getTitle();
			?>
			<div class="basket top">
				<!-- noindex -->
				<a rel="nofollow" title="<?=$title_text?>" href="<?=$arTheme['BASKET_PAGE_URL']?>" class="fill-theme-hover light-opacity-hover dark_link <?=$class_link.' '.$class_icon?>">
					<span class="js-basket-block header-cart__inner <?=($bEmptyBasket ? 'header-cart__inner--empty' : 'icon-block-with-counter--count')?>">
						<?=self::showSpriteIconSvg(SITE_TEMPLATE_PATH."/images/svg/catalog/item_icons.svg#cart-21-18", "basket header__icon banner-light-icon-fill menu-light-icon-fill", ['WIDTH' => 21,'HEIGHT' => 18]);?>
						<span class="header-cart__count bg-more-theme count<?=($bEmptyBasket ? ' empted' : '')?>"><?=$count?></span>
					</span>

					<?if(strlen($txt)):?>
						<span class="header__icon-name header-cart__name menu-light-text banner-light-text"><?=$txt?></span>
					<?endif;?>
				</a>
	...
	public static function ShowCompareLink($class_link = 'top-btn hover', $class_icon = '', $txt = ''){
		static $iCalledID;
		++$iCalledID;
	...
	public static function showFavoriteLink($class_link = '', $class_icon = '', $txt = ''){
		static $favorite_call;
		$iFavoriteID = ++$favorite_call;

		Bitrix\Main\Page\Frame::getInstance()->startDynamicWithID('favorite-link'.$iFavoriteID);
		
		$arItems = Aspro\Lite\Itemaction\Favorite::getItems();
		$count = count($arItems);
		$bEmptyFavorite = !$count;
		$title_text = Aspro\Lite\Itemaction\Favorite::getTitle();
		?>
		<a rel="nofollow" title="<?=$title_text?>" href="<?=SITE_DIR?>personal/favorite/" class="header-icon dark_link <?=$class_link?>">
			<span class="favorite-block icon-block-with-counter <?=$class_icon;?> fill-use-888 fill-theme-use-svg-hover">
				<span class="js-favorite-block <?=(!$bEmptyFavorite ? 'icon-block-with-counter--count' : '')?>">
					<span class="icon-count icon-count--favorite bg-more-theme count<?=($bEmptyFavorite ? ' empted' : '')?>"><?=$count?></span>
				</span>
				<?=self::showSpriteIconSvg(SITE_TEMPLATE_PATH.'/images/svg/catalog/item_icons.svg#favorite-18-16', 'header__icon banner-light-icon-fill', ['WIDTH' => 18,'HEIGHT' => 16]);?>
			</span>
			<?if($txt):?>
				<span class="header__icon-name"><?=$txt?></span>
			<?endif;?>
		</a>
		<div class="favorite-dropdown product-dropdown-hover-block"></div>
		<?
		Bitrix\Main\Page\Frame::getInstance()->finishDynamicWithID('favorite-link'.$iFavoriteID);
	}
		...
		if(
			...
			<div class="mobilemenu__menu mobilemenu__menu--cart">
				<ul class="mobilemenu__menu-list">
					<li class="mobilemenu__menu-item mobilemenu__menu-item--with-icon<?=(CLite::IsBasketPage() ? ' mobilemenu__menu-item--selected' : '')?>">
						<div class="link-wrapper bg-opacity-theme-parent-hover fill-theme-parent-all color-theme-parent-all fill-dark-light-block fill-theme-use-svg-hover basket">
							<a class="dark_link icon-block" href="<?=$arTheme['BASKET_PAGE_URL']?>" rel="nofollow" title="<?=$title_text?>">
								<span class="js-basket-block header-cart__inner mobilemenu__menu-item-svg <?=($bEmptyBasket ? 'header-cart__inner--empty' : 'icon-block-with-counter--count')?>">
									<?= self::showSpriteIconSvg(SITE_TEMPLATE_PATH.'/images/svg/catalog/item_icons.svg#cart-21-18', 'basket fill-theme-target mobilemenu__menu-item-svg', ['WIDTH' => 21, 'HEIGHT' => 18]); ?>
									<span class="header-cart__count bg-more-theme count<?=($bEmptyBasket ? ' empted' : '')?>"><?=$count?></span>
								</span>
	...
	public static function ShowMobileMenuCompare(){
		...
		if(
			...
			$arItems = Aspro\Lite\Itemaction\Compare::getItems();
			$count = count($arItems);
			$bEmptyCompare = !$count;
			$title_text = Aspro\Lite\Itemaction\Compare::getTitle();
			?>
			<div class="mobilemenu__menu mobilemenu__menu--compare">
				<ul class="mobilemenu__menu-list">
					<li class="mobilemenu__menu-item mobilemenu__menu-item--with-icon<?=(CLite::IsComparePage() ? ' mobilemenu__menu-item--selected' : '')?>">
						<div class="link-wrapper bg-opacity-theme-parent-hover fill-theme-parent-all color-theme-parent-all fill-dark-light-block fill-theme-use-svg-hover">
							<a class="icon-block icon-block-with-counter dark_link" href="<?=$arTheme['COMPARE_PAGE_URL']?>" rel="nofollow" title="<?=$title_text?>">
								<span class="icon-block-with-counter__inner mobilemenu__menu-item-svg js-compare-block <?=(!$bEmptyCompare ? 'icon-block-with-counter--count' : '')?>">
									<?=self::showSpriteIconSvg(SITE_TEMPLATE_PATH."/images/svg/catalog/item_icons.svg#compare-20-16", "compare", ['WIDTH' => 20,'HEIGHT' => 16]);?>
	...
	public static function ShowMobileMenuFavorite(){
		static $iCalledID;
		++$iCalledID;

		Bitrix\Main\Page\Frame::getInstance()->startDynamicWithID('mobilemenu__favorite'.$iCalledID);
		...
		if(
			...
			<div class="mobilemenu__menu mobilemenu__menu--favorite">
				<ul class="mobilemenu__menu-list">
					<li class="mobilemenu__menu-item mobilemenu__menu-item--with-icon<?=(self::IsFavoritePage() ? ' mobilemenu__menu-item--selected' : '')?>">
						<div class="link-wrapper bg-opacity-theme-parent-hover fill-theme-parent-all color-theme-parent-all fill-dark-light-block fill-theme-use-svg-hover">
							<a class="icon-block icon-block-with-counter dark_link" href="<?=$arTheme['FAVORITE_PAGE_URL']?>" rel="nofollow" title="<?=$title_text?>">
								<span class="icon-block-with-counter__inner mobilemenu__menu-item-svg js-favorite-block <?=(!$bEmptyFavorite ? 'icon-block-with-counter--count' : '')?>">
									<?=self::showSpriteIconSvg(SITE_TEMPLATE_PATH."/images/svg/catalog/item_icons.svg#favorite-18-16", "favorite", ['WIDTH' => 18, 'HEIGHT' => 16]);?>
									<span class="icon-count icon-count--favorite bg-more-theme count<?=($bEmptyFavorite ? ' empted' : '')?>"><?=$count?></span>
								</span>
								<span class="font_15"><?=GetMessage('FAVORITE_TEXT')?></span>
							</a>
						</div>
					</li>
				</ul>
		...
		Bitrix\Main\Page\Frame::getInstance()->finishDynamicWithID('mobilemenu__favorite'.$iCalledID);
	}
	...
	public static function ShowMobileMenuRegions(){
		...
		if($arRegion){
			...
			$arRegions = CLiteRegionality::getRegions();
			...
			<div class="mobilemenu__menu mobilemenu__menu--regions">
				<ul class="mobilemenu__menu-list">
					<li class="mobilemenu__menu-item mobilemenu__menu-item--with-icon mobilemenu__menu-item--parent">
						<div class="link-wrapper">
							<a class="icon-block dark_link bg-opacity-theme-parent-hover fill-theme-parent-all color-theme-parent-all fill-dark-light-block fill-theme-use-svg-hover" href="" title="<?=htmlspecialcharsbx($arSite['LANG'])?>">
								<?= self::showSpriteIconSvg(SITE_TEMPLATE_PATH.'/images/svg/header_icons.svg#region-17-16', 'region mobilemenu__menu-item-svg fill-theme-target', ['WIDTH' => 17, 'HEIGHT' => 16]); ?>
								
								<span class="icon-block__content">
									<span class="font_15"><?=$arRegion['NAME']?></span>
									<?= self::showSpriteIconSvg(SITE_TEMPLATE_PATH.'/images/svg/arrows.svg#right-7-5', 'down menu-arrow fill-theme-target fill-dark-light-block', ['WIDTH' => 7, 'HEIGHT' => 5]); ?>
								</span>
						...
						<ul class="mobilemenu__menu-dropdown dropdown">
							...
							<li class="mobilemenu__menu-item mobilemenu__menu-item--title">
								<div class="link-wrapper">
									<a class="dark_link" href="">
										<span class="font_18 font_bold"><?=Loc::getMessage('LITE_T_MENU_REGIONS')?></span>
									</a>
								</div>
							</li>
	...
	public static function checkContentBlock($file, $prop = 'PROPERTY_ADDRESS_VALUE'){
		global $arRegion;
		if((CLite::checkContentFile($file) && !$arRegion) || ($arRegion && $arRegion[$prop]))
			return true;
		return false;
	}
	...
	public static function getDefaultThematic() {
		return self::isSaleMode() ? 'SHOP' : 'START';
	}

	public static function getCurrentThematic($SITE_ID = ''){
		$SITE_ID = strlen($SITE_ID) ? $SITE_ID : (defined('SITE_ID') ? SITE_ID : '');

		return Option::get(self::moduleID, 'THEMATIC', self::getDefaultThematic(), $SITE_ID);
	}
	...
	public static function CheckAdditionalChainInMultiLevel(&$arResult, &$arParams, &$arElement){
		...
		if($arParams['INCLUDE_IBLOCK_INTO_CHAIN'] == 'Y' && isset(CLiteCache::$arIBlocksInfo[$arParams['IBLOCK_ID']]['NAME']))
			$APPLICATION->AddChainItem(CLiteCache::$arIBlocksInfo[$arParams['IBLOCK_ID']]['NAME'], $arElement['~LIST_PAGE_URL']);

		if($arParams['ADD_SECTIONS_CHAIN'] == 'Y')
		{
			if($arSection = CLiteCache::CIBlockSection_GetList(array('CACHE' => array('TAG' => CLiteCache::GetIBlockCacheTag($arElement['IBLOCK_ID']), 'MULTI' => 'N')), self::GetCurrentSectionFilter($arResult['VARIABLES'], $arParams), false, array('ID', 'NAME')))
			{
	...
	public static function CheckDetailPageUrlInMultilevel(&$arResult){
		if($arResult['ITEMS']){
			...
			$arItems = CLiteCache::CIBLockElement_GetList(array('CACHE' => array('TAG' => CLiteCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => array('ID'), 'MULTI' => 'N')), array('ID' => $arItemsIDs), false, false, array('ID', 'IBLOCK_SECTION_ID', 'DETAIL_PAGE_URL'));
	...
	public static function Start() {
		...
		// update theme values
		self::UpdateFrontParametrsValues();
		...
		// PWA manifest
		\Aspro\Lite\PWA::showMeta(SITE_ID);

		self::setCssVariables($arTheme);

		// register js and css libs
		\Aspro\Lite\Functions\Extensions::register();
		...
		if ($arTheme['USE_BOOTSTRAP'] === 'Y') {
			\Aspro\Lite\Functions\Extensions::init('bootstrap');
		} else {
		...
		if($arTheme['USE_LAZY_LOAD'] === 'Y' && !self::checkMask(Option::get(self::moduleID, 'LAZY_LOAD_EXCEPTIONS', '')) ){
		...
		CAjax::Init();
		\Aspro\Lite\Functions\Extensions::init(['metrika.goals', 'logo']);
		self::addExtensionsByCondition();
		self::addCodeByCondition();

		// show wizard panel button
		\Aspro\Lite\Panel\Button::getInstance([
			'siteId' => SITE_ID,
	...
	public static function setCssVarFromBuffer(){
		global $APPLICATION;
		$cssBlock = $APPLICATION->AddBufferContent(array('CLite', 'showCssVarFromBuffer'));
		// $GLOBALS['APPLICATION']->AddHeadString($cssBlock);
		echo $cssBlock;
	}
	...
	public static function addExtensionsByCondition(){
		// get site options
		$arTheme = self::GetFrontParametrsValues(SITE_ID);
		...
		if(
			...
			/* item action & notice */
			\Aspro\Lite\Functions\Extensions::init('item_action');
			\Aspro\Lite\Functions\Extensions::init('notice');
		}
		elseif ($arTheme['CABINET'] === 'Y') {
			/* only notice */
			\Aspro\Lite\Functions\Extensions::init('notice');
		}
		...
		if (self::IsBasketPage()) {
			\Aspro\Lite\Functions\Extensions::init('basket');
			if($arTheme['SHOW_SHARE_BASKET'] === 'Y'){
				\Aspro\Lite\Functions\Extensions::init('share');
			}
		}

		/* order */
		if (self::IsOrderPage()) {
			\Aspro\Lite\Functions\Extensions::init('order');
		}

		/* personal */
		if (self::IsPersonalPage()) {
			\Aspro\Lite\Functions\Extensions::init('personal');
		}

		/* scroll_to_top */
		if($arTheme['SCROLLTOTOP_TYPE'] !== 'NONE'){
			\Aspro\Lite\Functions\Extensions::init('scroll_to_top');
		}
	}
	
	public static function addCodeByCondition(){
		/* basket */
		if (self::IsBasketPage()) {
			Aspro\Lite\Functions\Basket::showPriceFormat();
		}
	}
	...
	public static function ShowPageProps($prop){
		...
		global $APPLICATION;
		$APPLICATION->AddBufferContent(array("CLite", "GetPageProps"), $prop);
	}
   ...
   protected static function IsComponentTemplateHasModuleElementsPageBlocksParam($templateName, $arExtParams = array()){
    ...
    public static function GetItemsYear($arParams){
    	$arResult = array();
    	$arItems = CLiteCache::CIBLockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CLiteCache::GetIBlockCacheTag($arParams['IBLOCK_ID']))), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'ACTIVE' => 'Y'), false, false, array('ID', 'NAME', 'ACTIVE_FROM'));
		if($arItems)
		{
			foreach($arItems as $arItem)
			{
	...
	public static function GetBasketPage(){
		...
		return strlen($arOptions['BASKET_PAGE_URL']) ? $arOptions['BASKET_PAGE_URL'] : SITE_DIR.'basket/';
	}
	...
	public static function IsBasketPage($page = ''){
		...
		if (strlen($page)) {
			return CSite::InDir($page);
		}
		else {
			if (!isset($result)) {
				$page = self::GetBasketPage();
	...
	public static function IsFavoritePage($url_link = ''){
		static $result;
		...
		if(!isset($result))
		{
			...
			{
				$arOptions = self::GetBackParametrsValues(SITE_ID);
				if(!strlen($arOptions["FAVORITE_PAGE_URL"]))
					$arOptions["FAVORITE_PAGE_URL"] = SITE_DIR."personal/favorite/";
				$url_link = $arOptions["FAVORITE_PAGE_URL"];
			}
			$result = CSite::InDir($url_link);
		}
	...
	public static function GetPersonalPage(){
		$arOptions = self::GetBackParametrsValues(SITE_ID);

		return strlen($arOptions['PERSONAL_PAGE_URL']) ? $arOptions['PERSONAL_PAGE_URL'] : SITE_DIR.'personal/';
	}
	...
	public static function GetCatalogPage(){
		...
		return strlen($arOptions['CATALOG_PAGE_URL']) ? $arOptions['CATALOG_PAGE_URL'] : SITE_DIR.'catalog/';
	}
	...
	public static function IsFormPage($page = ''){
		static $result;
	...
	public static function IsBlogPage($page = ''){
		static $result;

		if (!isset($result)) {
			if (!$page) {
				$page = SITE_DIR.'articles/';
			}

			$result = CSite::InDir($page) || ($GLOBALS['APPLICATION']->GetProperty('BLOG_PAGE') === 'Y');
		}
	...
	public static function getConditionClass(){
		...
		$class = $APPLICATION->AddBufferContent(array('CLite', 'showPageClass'));
		...
		elseif (self::IsBasketPage() || self::IsOrderPage()) {
			$class .= ' simple_basket_mode';
		}
		...
		$class .= $arSiteThemeOptions['ORDER_VIEW'] === 'Y' ? ' with_order' : '';
		...
		<?if($bImg):?>
			<?if($bFromRegion):?>
				...
				<?if($bGallery && $arPhotos):?>
					<?\Aspro\Lite\Functions\Extensions::init(['fancybox', 'swiper']);?>
					<?
							...
							<div class="text-center gallery-big swiper-nav-offset">
								...
								<?if ($arOptions['countSlides'] > 1):?>
                                    <div class="slider-nav swiper-button-next">
                                        <?= self::showSpriteIconSvg(SITE_TEMPLATE_PATH . '/images/svg/arrows.svg#right-7-12', 'stroke-dark-light', [
                                            'WIDTH' => 7, 
                                            'HEIGHT' => 12
                                        ]); ?>
                                    </div>
                                    <div class="slider-nav swiper-button-prev">
                                        <?= self::showSpriteIconSvg(SITE_TEMPLATE_PATH . '/images/svg/arrows.svg#left-7-12', 'stroke-dark-light', [
                                            'WIDTH' => 7, 
                                            'HEIGHT' => 12
                                        ]); ?>
                                    </div>
	...
	public static function showContactAddr($txt = '', $wrapTable = true, $class = '', $icon = 'Addres_black.svg'){
		...
		if($bFromRegion){
			...
			$bAddr = self::checkContentFile(SITE_DIR.'include/contacts-site-address.php');
		}
		?>
		<?if($arRegion):?>
		...
		<?if($bAddr):?>
			<div class="contact-property contact-property--address">
				...
				<?if($bFromRegion):?>
					<div itemprop="address" class="contact-property__value <?=($class ? $class : 'color_222')?>">
						<?=$arRegion['PROPERTY_ADDRESS_VALUE']['TEXT'];?>
					</div>
				<?else:?>
					<div itemprop="address" class="contact-property__value <?=($class ? $class : 'color_222')?>"><?$APPLICATION->IncludeFile(SITE_DIR."include/contacts-site-address.php", Array(), Array("MODE" => "html", "NAME" => "address"));?></div>
				<?endif;?>
			</div>
		<?endif;?>
		<?if($arRegion):?>
	...
	public static function showContactSchedule($txt = '', $wrapTable = true, $class = '', $icon = '', $subclass = ''){
		...
		$bContent = false;
		if($bFromRegion){
			if (is_array($arRegion['PROPERTY_SHCEDULE_VALUE']) && $arRegion['PROPERTY_SHCEDULE_VALUE']['TEXT']) {
				$bContent = true;
			}
		} else{
			$bContent = self::checkContentFile(SITE_DIR.'include/contacts-site-schedule.php');
		}
		?>
		<?if($arRegion):?>
			<?$frame = new \Bitrix\Main\Page\FrameHelper('header-allcaddr-block'.$iCalledID);?>
		...
		<?if($bContent):?>
			<div class="contact-property contact-property--schedule">
				<div class="contact-property__label font_13 color_999"><?=$txt;?></div>
				<?if($bFromRegion):?>
					<div class="contact-property__value <?=($class ? $class : 'color_222')?>">
						<?=($arRegion['PROPERTY_SHCEDULE_VALUE']['TYPE'] === 'TEXT' ? nl2br($arRegion['PROPERTY_SHCEDULE_VALUE']['TEXT']) : $arRegion['PROPERTY_SHCEDULE_VALUE']['TEXT']);?>
					</div>
				<?else:?>
					<div class="contact-property__value <?=($class ? $class : 'color_222')?>"><?$APPLICATION->IncludeFile(SITE_DIR."include/contacts-site-schedule.php", Array(), Array("MODE" => "html", "NAME" => "schedule"));?></div>
				<?endif;?>
			</div>
		<?endif;?>
		<?if($arRegion):?>
	...
	public static function showContactDesc(){
		...
		$bDesc = false;
		if($bFromRegion){
			if (is_array($arRegion['PROPERTY_REGION_TAG_CONTACT_TEXT_VALUE']) && $arRegion['PROPERTY_REGION_TAG_CONTACT_TEXT_VALUE']['TEXT']) {
				$bDesc = true;
			}
		} else{
			$bDesc = self::checkContentFile(SITE_DIR.'include/contacts-site-about.php');
		}
		?>
		<?if($arRegion):?>
	...
	public static function showSchedule($arOptions = array()){
		...
		global $arRegion, $APPLICATION;
		$bBlock = ($arRegion ? ($arRegion['PROPERTY_SHCEDULE_VALUE']['TEXT'] ?? false): self::checkContentFile(SITE_DIR.'include/contacts-site-schedule.php'));
		?>
		<?if($arRegion):?>
		...
		<?if($arRegion):?>
			<?if($arRegion['PROPERTY_ADDRESS_VALUE']):?>
				...
				<div <?=($arOptions['CLASS'] ? 'class="'.$arOptions['CLASS'].'"' : '')?>>
					...
					<?endif;?>
					<div class="address__text font_short <?=$arOptions['FONT_SIZE'] ? 'font_'.$arOptions['FONT_SIZE'] : ''?> <?=$arOptions['LARGE'] ? 'address__text--large' : ''?> <?=$arOptions['NO_LIGHT'] ? '' : 'banner-light-text menu-light-text'?>">
						<?=$arRegion['PROPERTY_ADDRESS_VALUE']['TEXT'];?>
					</div>
				</div>
	...
	public static function showRightDok(){
		...
		?>
		<?if($callbackBlock || $questionBlock || $reviewBlock):?>
			<div class="right_dok">
				<?if($callbackBlock):?>
					<span class="link fill-theme-hover" title="<?=GetMessage("S_CALLBACK")?>">
						<span class="animate-load" data-event="jqm" data-param-id="<?=self::getFormID("aspro_lite_callback");?>" data-name="callback"><?=self::showIconSvg("call", SITE_TEMPLATE_PATH."/images/svg/back_call.svg");?></span>
					</span>
				<?endif;?>
				<?if($reviewBlock):?>
					<span class="link fill-theme-hover" title="<?=GetMessage("S_FEEDBACK")?>">
						<span class="animate-load" data-event="jqm" data-param-id="<?=self::getFormID("aspro_lite_feedback");?>" data-name="review"><?=self::showIconSvg("review", SITE_TEMPLATE_PATH."/images/svg/send_review.svg");?></span>
					</span>
				<?endif;?>
				<?if($questionBlock):?>
					<span class="link fill-theme-hover" title="<?=GetMessage("S_QUESTION")?>">
						<span class="animate-load" data-event="jqm" data-param-id="<?=self::getFormID("aspro_lite_question");?>" data-name="question"><?=self::showIconSvg("ask", SITE_TEMPLATE_PATH."/images/svg/ask_question.svg");?></span>
					</span>
				<?endif;?>
			</div>
		<?endif;?>
	...
	public static function getSectionsIds_NotInRegion($iblockId = false, $regionId = false){
		...
		if(!$iblockId){
			$iblockId = CLiteCache::$arIBlocks[SITE_ID]['aspro_lite_catalog']['aspro_lite_catalog'][0];
		}
			...
			if($arIblockHasUFRegion[$iblockId]){
				...
				if($regionId){
					...
					if(!isset($arCache[$iblockId][$regionId])){
						if($arSections = CLiteCache::CIBLockSection_GetList(
							array(
								'CACHE' => array(
									'TAG' => CLiteCache::GetIBlockCacheTag($iblockId),
									'MULTI' => 'Y'
							...
							if($arSectionsIds){
								if($arSectionsIds_ = CLiteCache::CIBLockSection_GetList(
									array(
										'CACHE' => array(
											'TAG' => CLiteCache::GetIBlockCacheTag($iblockId),
											'MULTI' => 'Y',
	...
	public static function checkElementsIdsInRegion(&$arIds, $iblockId = false, $regionId = false){
		...
		if($bFilterItem && $arIds){
			...
			if($iblockId){
				...
				if($regionId){
					if($arSectionsIds = self::getSectionsIds_NotInRegion($arFilter['IBLOCK_ID'], $regionId)){
						$arIds = CLiteCache::CIBLockElement_GetList(
							array(
								'CACHE' => array(
									'TAG' => CLiteCache::GetIBlockCacheTag($iblockId),
									'RESULT' => array('ID'),
			...
			<div class="row" data-SID="<?=$FIELD_SID?>">
				<div class="col-md-12 <?=(in_array($arQuestion['STRUCTURE'][0]['FIELD_TYPE'], array("checkbox", "radio")) ? "style_check bx_filter" : "");?>">
					...
					<div class="form-group <?=(!in_array($arQuestion['STRUCTURE'][0]['FIELD_TYPE'], array("file", "image", "checkbox", "radio", "multiselect", "date1")) ? "fill-animate" : "");?>">
						<label class="font_14" for="<?=$type.'_'.$FIELD_SID?>"><span><?=$arQuestion["CAPTION"]?><?=($arQuestion["REQUIRED"] == "Y" ? '&nbsp;<span class="required-star">*</span>' : '')?></span></label>
						<div class="input <?=(($arQuestion['STRUCTURE'][0]['FIELD_TYPE'] == 'date') ? 'dates' : '')?>">
							...
							?>
							<?if($FIELD_SID == 'RATING'):?>
								<div class="votes_block with-text flexbox flexbox--row flexbox--wrap">
									<div class="rating">
										<?for($i=1;$i<=5;$i++):?>
											<div class="item-rating pointer" data-message="<?=GetMessage('RATING_MESSAGE_'.$i)?>"><?=static::showIconSvg("star", SITE_TEMPLATE_PATH."/images/svg/star.svg");?></div>
										<?endfor;?>
									</div>
		...
		if($code)
		{
			if(self::GetFrontParametrValue('USE_BITRIX_FORM') == 'Y' && \Bitrix\Main\Loader::includeModule('form'))
			{
				...
				if($item = $rsForm->Fetch())
					...
					$form_id = CLiteCache::$arIBlocks[$site]["aspro_lite_form"][$code][0];
			}
			else
			{
				$form_id = CLiteCache::$arIBlocks[$site]["aspro_lite_form"][$code][0];
	...
	public static function setFonts($arTheme) {
		...
		$fontSize = $fontFamily = $fontTitleFamily = '';
		...
		if(!$arTheme['CUSTOM_FONT']){
			...
			$fontSize = self::$arParametrsList['MAIN']['OPTIONS']['FONT_STYLE']['LIST'][$arTheme['FONT_STYLE']]['VALUE'];
			$fontFamily = self::$arParametrsList['MAIN']['OPTIONS']['FONT_STYLE']['LIST'][$arTheme['FONT_STYLE']]['GROUP'];
			...
			$fontSize = floatval($arTheme["CUSTOM_FONT_SIZE"]).'px';
		...
		if($arTheme['TITLE_FONT'] == 'Y'){
			...
			if($arTheme['TITLE_FONT_STYLE']){
				...
				$fontTitleFamily = $title_font['TITLE'];
		...
		self::setFontsVariables([
			'FONT_SIZE' => $fontSize,
			'FONT_FAMILY' => $fontFamily,
			'FONT_TITLE_FAMILY' => $fontTitleFamily,
			'FONT_TITLE_WEIGHT' => $arTheme["FONT_TITLE_WEIGHT"],
		]);
	}

	public static function setFontsVariables($arOptions) {
		$css = '';
		$arVariables = [
			'font-size' => str_replace(' ', '', $arOptions['FONT_SIZE']),
			'--theme-font-family' => $arOptions['FONT_FAMILY'],
			'--theme-font-title-family' => $arOptions['FONT_TITLE_FAMILY'] ? $arOptions['FONT_TITLE_FAMILY'] : 'var(--theme-font-family)',
			'--theme-font-title-weight' => $arOptions['FONT_TITLE_WEIGHT'],
		];
		foreach ($arVariables as $property => $value) {
			$css .= $property.':'.$value.';';
		}
	...
	public static function getThemeColorsValues($siteId = '', $customColors = []) {
		$siteId = strlen($siteId) ? $siteId : (defined('SITE_ID') ? SITE_ID : '');
		$arThemeValues = self::GetFrontParametrsValues($siteId);

		if (
			$customColors &&
			is_array($customColors) &&
			array_key_exists('baseColor', $customColors)
		) {
			$isBaseCustom = $isMoreCustom = true;

			$baseColor = trim($customColors['baseColor']);
			$moreColor = array_key_exists('moreColor', $customColors) ? trim($customColors['moreColor']) : $baseColor;
			
			$isMoreEnabled = $moreColor != $baseColor;
		} else {
		...
		$outerBorderRadius = $arThemeValues['OUTER_BORDER_RADIUS'] ?? '12px';
		$textTransform = 'none';
		...
		return [
			...
			'--theme-more-color' => $moreColor,
			'--theme-outer-border-radius' => $outerBorderRadius,
			'--theme-text-transform' => $textTransform,
			'--theme-letter-spacing' => $letterSpacing,
			'--theme-button-font-size' => $buttonFontSize,
			'--theme-button-padding-2px' => $buttonPadding2px,
			'--theme-button-padding-1px' => $buttonPadding1px,
			'--theme-more-color-hue' => $hueMoreColor,
			'--theme-more-color-saturation' => $saturationMoreColor.'%',
			'--theme-more-color-lightness' => $lightnessMoreColor.'%',
			'--theme-base-color-hue' => $hueColor,
			'--theme-base-color-saturation' => $saturationColor.'%',
			'--theme-base-color-lightness' => $lightnessColor.'%',
			'--theme-lightness-hover-diff' => $lightnessHoverDiff.'%',
		];
	}

	public static function setThemeColorsValues($siteId = '', $customColors = []) {
		$siteId = strlen($siteId) ? $siteId : (defined('SITE_ID') ? SITE_ID : '');

		$css = '';
		foreach (self::getThemeColorsValues($siteId, $customColors) as $property => $value) {
			$css .= $property.':'.$value.';';
		}
	...
	static public function nlo($code, $attrs = ''){
		...
		if(!isset($arAvailable)){
			$arAvailable = array(
				'menu-fixed' => true,
				'menu-mobile' => !(isset($_SESSION['SESS_INCLUDE_AREAS']) && $_SESSION['SESS_INCLUDE_AREAS']) && !(defined('TEMPLATE_TYPE') && TEMPLATE_TYPE === 'mobile'),
				'menu-megafixed' => true,
				'menu-catalog' => true,
				'menu-header' => $GLOBALS['arTheme']['NLO_MENU']['VALUE'] === 'Y',
			);
		...
		if(
			...
			if(
				...
				$GLOBALS['APPLICATION']->editArea->includeAreaIndex = array(0 => 1001);
	...
	public static function getJQuerySrc() {
		static $files;
