...
if(!class_exists('CLiteCache')){
	class CLiteCache {
		...
		public static function CSaleBasket_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CLiteCache/sale/CSaleBasket_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		public static function CCatalogStore_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CLiteCache/catalog/CCatalogStore_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		public static function CForumMessage_GetListEx($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $iNum = 0, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CLiteCache/forum/CForumMessage_GetListEx/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		private static function _InitCacheParams($moduleName, $functionName, $arCache){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CLiteCache/".$moduleName."/".$functionName."/".$cacheTag."/";
			}
		...
		public static function ClearTagIBlock($arFields){
			...
			if($arFields["CODE"] && strpos($arFields["CODE"], 'aspro_lite_sku') !== false){
				$dirSkuCache = "/cache/CLiteCache/iblock/getSKUjs/";
				$obCache = new CPHPCache();
		...
		public static function ClearTagIBlockElement($arFields){
			...
			if($arFields["IBLOCK_ID"]){
				...
				if($arFields["ID"] && is_string($code) && strpos($code, 'aspro_lite_sku') !== false){
		...
		public static function ClearSKUjsCache($offerID){
			$dirSkuCache = "/cache/CLiteCache/iblock/getSKUjs/element_".$offerID;
	...
	// initialize CLiteCache::$arIBlocks array
	if(CLiteCache::$arIBlocks === NULL){
		CLiteCache::$bEnabled = COption::GetOptionString('main', 'component_managed_cache_on', 'Y') === 'Y';
		$arIBlocksTmp = CLiteCache::CIBlock_GetList(array("CACHE" => array("TAG" => "iblocks")), array("ACTIVE" => "Y", "CHECK_PERMISSIONS" => "N"));
		CLiteCache::$arIBlocks = CLiteCache::GroupArrayBy($arIBlocksTmp, array("GROUP" => array("LID", "IBLOCK_TYPE_ID", "CODE"), "MULTI" => "Y", "RESULT" => array("ID")));
		CLiteCache::$arIBlocksInfo = CLiteCache::GroupArrayBy($arIBlocksTmp, array("GROUP" => array("ID")));
	}
}