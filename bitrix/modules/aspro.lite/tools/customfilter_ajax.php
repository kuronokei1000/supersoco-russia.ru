...
if($action){
	if($action === 'init' || $action === 'save'){
		...
		$eventManager->addEventHandlerCompatible(
			...
			'OnCondCatControlBuildList', 
			array(
				'\Aspro\Lite\CCatalogCondCtrlIBlockFields',
				'GetControlDescr'
			)
		);
		...
		$eventManager->addEventHandlerCompatible(
			...
			'OnCondCatControlBuildList', 
			array(
				'\Aspro\Lite\CCatalogCondCtrlIBlockProps',
				'GetControlDescr'
			)
		);
		...
		$eventManager->addEventHandlerCompatible(
			...
			'OnCondCatControlBuildList', 
			array(
				'\Aspro\Lite\Property\CustomFilter\CondCtrl',
				'GetControlDescr'
			)
		);
		...
		if(check_bitrix_sessid() && $request->isPost()){
			...
			elseif($action === 'get_crosssales_iblockprops'){
				...
				if($propertyId > 0){
					...
					if($property){
						$arExcludeUserTypes = \Aspro\Lite\Property\CustomFilter\CondCtrl::getCrossSalesExcludePropertyUserTypes();
						...
						if($property['USER_TYPE'] != ''){
							if(!is_array($property['USER_TYPE_SETTINGS'])){
								...
								if(CheckSerializedData($property['USER_TYPE_SETTINGS']))
									$property['USER_TYPE_SETTINGS'] = Solution::unserialize($property['USER_TYPE_SETTINGS'], ['allowed_classes' => false]);
								if(!is_array($property['USER_TYPE_SETTINGS']))
