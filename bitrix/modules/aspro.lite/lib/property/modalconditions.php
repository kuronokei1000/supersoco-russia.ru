...
class ModalConditions{
	static function OnIBlockPropertyBuildList(){
		return array(
			'PROPERTY_TYPE' => 'S',
			'USER_TYPE' => 'SAsproModalConditionsLite',
			'DESCRIPTION' => Loc::getMessage('LITE_MODAL_CONDITIONS_PROP_TITLE'),
	...
	static function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName){
		...
		if($bDetailPage){
			if(!isset($cache)){
				$cache = array();
				$jsFile = '/bitrix/js/aspro.lite/property/modalconditions_control.js';
				$ajaxFile = '/bitrix/tools/aspro.lite/modalconditions_ajax.php';
							...
							'if(iv){'.
								...
								'if(td){'.
									...
									'if(tdd){'.
										...
										'}'.
									'}'.
									'initAsproLiteModalConditionsControl({'.
										'data: \'{"iblockId": 1}\','.
										'oCont: td,'.
										'oInput: iv,'.
										...
										'propertyParams: {'.
											...
											'JS_EVENT: \'initAsproLiteModalConditionsControl\','.
											'AJAX_FILE: \''.$ajaxFile.'\','.
