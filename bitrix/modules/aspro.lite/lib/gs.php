		...
		if(
			...
			if(in_array('wizard_solution', $arParts)){
				...
				if($bOptionSiteTableExists){
					$options = $connection->query("(SELECT MODULE_ID,NAME,SITE_ID,VALUE FROM b_option WHERE MODULE_ID = '".self::getModuleId()."') UNION (SELECT MODULE_ID,NAME,SITE_ID,VALUE FROM b_option_site WHERE MODULE_ID = '".self::getModuleId()."')");
				}
				else{
					$options = $connection->query("SELECT MODULE_ID,NAME,SITE_ID,VALUE FROM b_option WHERE MODULE_ID = '".self::getModuleId()."'");
				}
