...
use	CLite as Solution,
	CLiteCache as Cache,
	CLiteCondition as Condition,
	\Aspro\Lite\Property\CustomFilter,
	\Bitrix\Main\Web\Json;

class CrossSales {
	const ALL_USERS_GROUP_ID = 2;
	const IBLOCK_TYPE = 'aspro_lite_catalog';
	const IBLOCK_CODE = 'aspro_lite_cross_sales';
	...
	public function __set($name, $value){
		switch($name){
			case 'productId':
				...
				break;
			case 'arParams':
				$this->setProduct($this->productId, $this->arParams);
				break;
			case 'siteId':
				$this->siteId = $value;
							...
							if($obCache->InitCache($cacheTime, $cacheID, $cachePath)){
								...
								foreach($arRulesTmp as $i => &$arRule){
									if(in_array($arRule['ID'], $arRulesIDs)){
										// fix for iblock ver 2.0
										if (!isset($arRule['PROPERTY_SHOW_PLACE_ENUM_ID'])) {
											$arRule['PROPERTY_SHOW_PLACE_ENUM_ID'] = array_keys($arRule['PROPERTY_SHOW_PLACE_VALUE']);
										}
