			...
			if(Option::get(Solution::moduleID, 'PWA_USE', 'N', $siteId) === 'Y'){
				...
				foreach(array(
					...
					if($value = Option::get(Solution::moduleID, 'PWA_ICON_'.$size, 'a:0:{}', $siteId)){
						if($arIcon = Solution::unserialize($value)){
							$arIcons[] = array(
								'id' => $arIcon[0],
								'size' => $size.'x'.$size,
							);
			...
			if($arValues['PWA_USE'] === 'Y'){
				...
				foreach(array(
					...
					if($arValues['PWA_ICON_'.$size]){
						if($arIcon = Solution::unserialize($arValues['PWA_ICON_'.$size])){
			...
			foreach($arBlock['OPTIONS'] as $optionCode => $arOption){
				...
				if($optionType !== 'note'){
					...
					elseif($optionTypeExt === 'colorpicker'){
						$baseColor = Option::get(Solution::moduleID, 'BASE_COLOR', Solution::$arParametrsList['MAIN']['OPTIONS']['BASE_COLOR']['DEFAULT'], $siteId);
						if($baseColor === 'CUSTOM'){
							$colorValue = Option::get(Solution::moduleID, 'BASE_COLOR_CUSTOM', Solution::$arParametrsList['MAIN']['OPTIONS']['BASE_COLOR_CUSTOM']['DEFAULT'], $siteId);
						}
						else{
							$colorValue = ltrim(Solution::$arParametrsList['MAIN']['OPTIONS']['BASE_COLOR']['LIST'][$baseColor]['COLOR'], '#');
						}
