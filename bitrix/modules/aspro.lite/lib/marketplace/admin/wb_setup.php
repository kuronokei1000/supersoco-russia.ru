...
if (($ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT_COPY') && $STEP == 1) {
    ...
    if (isset($arOldSetupVars['NEED_UPLOAD_IMAGES']))
        $needUploadImages = $arOldSetupVars['NEED_UPLOAD_IMAGES'];
...
//        }
    ...
    if (isset($_POST['NEED_UPLOAD_IMAGES']) && is_string($_POST['NEED_UPLOAD_IMAGES']))
        $needUploadImages = $_POST['NEED_UPLOAD_IMAGES'];
    if (!isset($needUploadImages) || $needUploadImages != 'Y')
        $needUploadImages = 'N';
...
<form method="post" action="<? echo $APPLICATION->GetCurPage() . $actionParams ?>" name="yandex_setup_form"
    ...
    if ($STEP == 1) {
        ...
        if (!isset($needUploadImages) || $needUploadImages != 'N')
            $needUploadImages = 'Y';
                    ...
                    function showDetailPopup() {
                        if (!obDetailWindow) {
                            ...
                            var obDetailWindow = new BX.CAdminDialog({
                                'content_url': '/bitrix/tools/aspro.lite/marketplace/wb_detail.php?lang=<?=LANGUAGE_ID?>&bxpublic=Y&IBLOCK_ID=' + s[s.selectedIndex].value,
                                'content_post': '&SECTIONS='+ JSON.stringify(selectedSectionIds) + '&XML_DATA=' + BX.util.urlencode(dat.value) + '&' + '<?echo bitrix_sessid_get(); ?>',
        ...
        <tr>
            ...
            <td width="60%">
                <input
                    type="hidden" name="CHECK_PERMISSIONS" value="N">
                <input
                    type="checkbox" name="CHECK_PERMISSIONS"
        ...
        <tr>
            ...
            <td width="60%">
                <input
                    type="hidden" name="FILTER_AVAILABLE" value="N">
                <input
                    type="checkbox" name="FILTER_AVAILABLE"
        ...
        <tr>
            <td width="40%"><?= GetMessage('AS_UPLOAD_IMAGES') ?></td>
            <td width="60%">
                <input
                    type="hidden" name="NEED_UPLOAD_IMAGES" value="N">
                <input
                    type="checkbox" name="NEED_UPLOAD_IMAGES"
                    value="Y"<? echo($needUploadImages == 'Y' ? ' checked' : ''); ?>>
            </td>
        </tr>
        ...
        if (($ACTION == "EXPORT_SETUP" || $ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT') && $PROFILE_ID > 0) {
            ...
            <td width="60%">
                <?
                $UrlToLog = '/upload/mp-export-log/wildberries_' . htmlspecialcharsbx($PROFILE_ID);
    ...
    if (2 > $STEP) {
        ...
        <input
            ...
            value="NEED_UPLOAD_PRICES,NEED_UPLOAD_STORES,V,IBLOCK_ID,SITE_ID,SETUP_SERVER_NAME,COMPANY_NAME,SETUP_FILE_NAME,XML_DATA,USE_HTTPS,FILTER_AVAILABLE,DISABLE_REFERERS,EXPORT_CHARSET,MAX_EXECUTION_TIME,CHECK_PERMISSIONS,NEED_UPLOAD_IMAGES">
        <input
            type="submit"
