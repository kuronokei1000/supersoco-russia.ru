...
BX.BitrixVue.component('mpm-property', {
    ...
    mounted() {
        if (this.mapData) {
            ...
            if(this.mapData['BX_SKU_PROPERTY']) {
                this.bxSkuPropertyCode = this.mapData['BX_SKU_PROPERTY'];
            }
        }
    ...
    methods: {
        ...
        onBxSelectSkuProperty(bxSkuPropertyCode) {
            this.bxSkuPropertyCode = bxSkuPropertyCode || '';
        ...
        showMatchingPropsValue(post = {}) {
            const obDetailWindow = new BX.CAdminDialog({
                ...
                'content_post': Object.assign({
                    ...
                    bxSkuCode: this.bxSkuPropertyCode,
                    // controller: this.$root.controller
                }, post),
...
BX.BitrixVue.component('mpm-control-select-sku-property', {
    props: {
        value: {
            type: String,
        }
    ...
    mounted() {
        setTimeout(() => {
            this.select2 = $(this.$refs.elSelect).select2({
                ...
                data: this.$root.propertySkuSelectOptions,
                templateResult: state => {
                    if ('id' in state && !state.id) {
                        return BX.message('VALUES_FROM_CATALOG');
                    }
                    return state.text;
                },
            })
        ...
        this.select2 = $(this.$refs.elSelect).select2({
            ...
            language: {
                inputTooShort: function () {
                    return BX.message('AS_SELECT2_TOO_SHORT_3');
                },
                searching: function () {
                    return BX.message('AS_SELECT2_SEARCHING');
                },
