...
class Ozon extends Base
{
     ...
     * @return array
     */
    public function getServiceCategories($all = false): array
    {
    ...
    public function getBuildItem(array $element): ?array
    {
        ...
        $item = array_filter([
            ...
            'offer_id' => strval($this->getValueFieldWithSku('vendor', $structure, $element)),
            'barcode' => strval($this->getValueFieldWithSku('barcode', $structure, $element)),
            'name' => $element['NAME'],
            'dimension_unit' => 'mm',
            'weight_unit' => 'g',
        ] + $arVariations + $arImgs);
    ...
    protected function getBuildScopeStructure($iblockSectionId): array
    {
        ...
        $vendorSkuPropertyCode = $mapCategory['properties'][self::REQUIRED_FIELD_VENDOR_CODE]['bx_sku_property_code'] ?? null;
        $barcodePropertyCode = $mapCategory['properties'][self::REQUIRED_FIELD_BARCODE]['bx_property_code'] ?? null;
        $barcodeSkuPropertyCode = $mapCategory['properties'][self::REQUIRED_FIELD_BARCODE]['bx_sku_property_code'] ?? null;
        $pricePropertyCode = $mapCategory['properties'][self::REQUIRED_FIELD_PRICE]['bx_property_code'] ?? null;
        $priceSkuPropertyCode = $mapCategory['properties'][self::REQUIRED_FIELD_PRICE]['bx_sku_property_code'] ?? null;
        ...
        $this->scopeStructures[$iblockSectionId] = [
            ...
            'vendor_sku_field' => $vendorSkuPropertyCode,
            'barcode_field' => $barcodePropertyCode,
            'barcode_sku_field' => $barcodeSkuPropertyCode,
            'price_field' => $pricePropertyCode,
            'price_sku_field' => $priceSkuPropertyCode,
    ...
    protected function checkRequiredNomenclatureValues(array $structure, array $element): bool
    {
        ...
        if ($structure['object']) {
            if ($arProps = array_filter($this->getServiceCategoryProperties($structure['object']), fn($arProp) => $arProp['is_required'])) {
                ...
                if ($this->isEmptyVendorField($structure, $element)) {
                ...
                foreach ($arProps as $arProp) {
                    if (
                        !$structure['all_properties'][$arProp['code']]
                         || (
                                $structure['all_properties'][$arProp['code']] 
                                && !$this->getValuePropdWithSku($arProp['code'], $structure, $element)
                            )
                        ) {
                ...
                if (!$this->getValueFieldWithSku('vendor', $structure, $element)) {
    ...
    {
        if (Helper::isOffer($element)) {
            $value = $element[$propertyStructure['bx_sku_property_code']]
                ?: $element[$propertyStructure['bx_property_code']]
        ...
        if ($propertyStructure['dictionary_id']) {
            if ($this->mappingPropertyValues[$propertyStructure['code']]) {
                ...
                foreach ($this->mappingPropertyValues[$propertyStructure['code']] as $code => $id) {
                    if (is_array($id)) {
    ...
    protected function getElementImages(array $structure, array $element): array
    {
        ...
        if ($imgs = $element[$structure['nomenclature_properties']['IMAGES']['bx_sku_property_code']]) {
    ...
    {
        $arResult = [];
        $priceCode = $this->scopeStructures[$element['IBLOCK_SECTION_ID']]['price_field'];
        $priceSkuCode = $this->scopeStructures[$element['IBLOCK_SECTION_ID']]['price_sku_field'];

        if ($priceSkuCode && $element[$priceSkuCode]) {
            $priceCode = $priceSkuCode;
        }
        $priceGroup = str_replace('CATALOG_PRICE_', '', $priceCode);
    ...
    private function getValueFieldWithSku(string $field, array $structure, array $element)
    {
        $value = '';
        if (isset($structure[$field.'_field'])) {
            $value = $element[$structure[$field.'_field']];
        }
        //sku
        if (Helper::isOffer($element) && isset($structure[$field.'_sku_field']) && $element[$structure[$field.'_sku_field']]) {
            $value = $element[$structure[$field.'_sku_field']];
        }
        //
    ...
    private function isEmptyVendorField(array $structure, array $element)
    {
        return (!$structure['vendor_field'] || (Helper::isOffer($element) && !$structure['vendor_field'] && !$structure['vendor_sku_field']));
    }

    private function getValuePropdWithSku(string $propCode, array $structure, array $element)
    {
        $value = $structure['all_properties'][$propCode]['bx_property_code'] ?: $structure['all_properties'][$propCode]['default_value'];

        //sku
        if (Helper::isOffer($element)) {
            $value = $structure['all_properties'][$propCode]['bx_sku_property_code'] ?: $value;
        }
        //
     ...
     * @return array
     */
    public function getServiceGoods($last_value_id = 0, $limit = 1000): array
    {
        $result = $this->service->getGoods($last_value_id, $limit);

        return $this->encoding($result) ?: [];
    }
    
    public function getServiceProductList($filter, $limit = 1000): array
    {
        $result = $this->service->getProductList($filter, $limit);
        ...
        if ($result['result']) {
            ...
            foreach ($result['result']['items'] as $item) {
                $tmp = [
                    ...
                    'visible' => $item['visible'] ? 'Y' : 'N',
                    'active_product' => $item['visibility_details']['active_product'],
                    'stocks' => $item['stocks'],
                ];

                foreach ($item['sources'] as $sources) {
                    if ($sources['is_enabled']) {
                        if (!$tmp['fbo'] && $sources['source'] === 'fbo') {
                            $tmp['fbo'] = $sources['sku'];
                        }
                        if (!$tmp['fbs'] && $sources['source'] === 'fbs') {
                            $tmp['fbs'] = $sources['sku'];
                        }
                    }
                }
