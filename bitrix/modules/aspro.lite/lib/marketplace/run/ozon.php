...
class Ozon
{
    ...
    /** @var array Данные для обновления цен. (элементы должны быть в Ozon) */
    private $itemsToPriceUpdate = [];

    /** @var array Данные для обновления остатков. (элементы должны быть в Ozon) */
    ...
    public function __construct($iblockId, $clientId = '', $token = '')
    {
        ...
        if ($arInfo['IBLOCK_ID'] && $arInfo['SKU_PROPERTY_ID']) {
            $this->finderOffers = new Finder($arInfo['IBLOCK_ID']);
            $this->finderOffers->skuPropertyId = $arInfo['SKU_PROPERTY_ID'];
        }
    }
    ...
    public function export(): array
    {
        ...
        foreach ($this->scopeElements as $element) {
            ...
            if ($element['TYPE'] == \Bitrix\Catalog\ProductTable::TYPE_SKU) {
                foreach ($this->getOffersByElement($element) as $offer) {
                    $this->fillOfferFieldsFromElement($offer, $element);

                    $this->addItem($offer);
                }
                $this->getStoresForOffers();
            } else {
    ...
    protected function getOffersByElement($element)
    {
        if ($this->finderOffers->elementIds) {
            $this->finderOffers->elementIds = [];
            $this->finderOffers->elements = [];
        }

        $filter = $this->getOffersFilter();

        $this->finderOffers->setOptions([
            'getPropValueId' => 'Y',
            'filter' => [
                'PROPERTY_' . $this->finderOffers->skuPropertyId => $element['ID'],
                $filter,
            ],
            'productType' => [
        ...
        return $this->finderOffers->getElements();
    }
    
    protected function getOffersFilter()
    {
        $filter = $this->getOffersFilterObject($this->finder->options['filter']);

        if (gettype($filter) === 'object') {
            if ($filter->arFilter && is_array($filter->arFilter)) {
                return $filter->arFilter;
                return $filter->arFilter[0];
            }
        }
        return [];
    }
    
    protected function getOffersFilterObject(array $arOptions)
    {
        $result = [];
        $filterKey = '=ID';
        ...
        if ($arOptions) {
            foreach ($arOptions as $key => $arOption) {
                ...
                if (is_array($arOption)) {
                    $result = $this->getOffersFilterObject($arOption);
                }
            }
        }
        return $result;
    ...
    protected function fillOfferFieldsFromElement(&$offer, $element)
    {
        $arFields = [
            'IBLOCK_SECTION_ID',
            'DETAIL_PICTURE',
        ...
        foreach ($arFields as $field) {
            if (!$offer[$field] && $element[$field]) {
                $offer[$field] = $element[$field];
            }
        }

        //add CML2_ARTICLE
        if (!$offer['CML2_ARTICLE'] && $offer['ARTICLE']) {
            $offer['CML2_ARTICLE'] = $offer['ARTICLE'];
        }

        //add PARENT_NAME
        $offer['PARENT_NAME'] = $element['NAME'];
    }

    protected function getStoresForOffers()
    {
        $this->scopeElementStores += $this->finderOffers->getElementStores();
    }
    ...
    protected function addItem($element)
    {
        ...
        $this->processProduct($element, $item);
    ...
    protected function processProduct($element, $item)
    {
