...
if(!class_exists('CSKU'))
{
	...
	class CSKU{
		 ...
		 * @var array treeProps Tree props by item
		...
		public function __construct(array $arOptions = [])
		{
			...
			$arOptions['SKU_SORT_FIELD'] = $arOptions['OFFERS_SORT_FIELD'] ?? $arOptions['SKU_SORT_FIELD'];
			$arOptions['SKU_SORT_ORDER'] = $arOptions['OFFERS_SORT_ORDER'] ?? $arOptions['SKU_SORT_ORDER'];
			$arOptions['SKU_SORT_FIELD2'] = $arOptions['OFFERS_SORT_FIELD2'] ?? $arOptions['SKU_SORT_FIELD2'];
			$arOptions['SKU_SORT_ORDER2'] = $arOptions['OFFERS_SORT_ORDER2'] ?? $arOptions['SKU_SORT_ORDER2'];
		...
		/**
		 * Set OID for offers detail page url
		 * 
		 */
		public function setDetailURL(string $url): void
		{
			if ($this->items) {
				$oid = Solution::GetFrontParametrValue('CATALOG_OID');
				foreach ($this->items as $key => $arItem) {
					if (!$arItem['DETAIL_PAGE_URL']) {
						$this->items[$key]['DETAIL_PAGE_URL'] = $url . '?' . $oid . '=' . $arItem['ID'];
					}
				}
			}
		}
			...
			if ($this->items && $this->config['SKU_PROPERTY_CODE']) {
				...
				if ($arProps) {
					foreach ($arProps as $key => $arProp) {
						if ($this->items[$key]) {
							foreach ($arProp as &$propValue) {
								if(!empty($propValue["VALUE"])){
									$displayProperty = \CIBlockFormatProperties::GetDisplayValue($this->items[$key], $propValue);
									$propValue['DISPLAY_VALUE'] = $displayProperty['DISPLAY_VALUE'];
								}
							}
			...
			while ($propInfo = $propertyIterator->fetch()) {
				...
				if ($propInfo['PROPERTY_TYPE'] == Iblock\PropertyTable::TYPE_STRING) {
					...
					if ($propInfo['USER_TYPE_SETTINGS'] == '')
						continue;
					$propInfo['USER_TYPE_SETTINGS'] = Solution::unserialize($propInfo['USER_TYPE_SETTINGS']);
