...
if(!class_exists("CAsproLiteReCaptcha"))
{
	class CAsproLiteReCaptcha{
		...
		public static function getCodeByPostList($arPost = array())
		{
			...
			$res = $connection->query("SELECT CODE FROM b_captcha WHERE id='" . $helper->forSql($captchaSid) . "'")->fetch();
			if($res['CODE'])
				return $res['CODE'];
		...
		public static function checkClientResponse($captchaResponse)
		{
			...
			catch(SystemException $exception)
			{
				\CEventLog::Add(array(
					"SEVERITY" => "WARNING",
					"AUDIT_TYPE_ID" => 'ASPRO_LITE.RECAPTCHA_ERROR',
