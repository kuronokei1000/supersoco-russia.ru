...
class Price {
    ...
    public static function check(array $arItem = [], array $arCondition = ['CAN_ACCESS' => 'Y'])
    {
        ...
        if (self::$catalogInclude) {
            $arPrice = !empty($arItem['PRICES']) ? self::getCatalogPrice($arItem['PRICES']) : self::getCatalogPriceNew($arItem);
            if(!empty($arPrice)){
    ...
    public static function getMinPriceNew(array $arPrices = [], array $arCondition = ['CAN_ACCESS' => 'Y'])
    {
        if (!is_array($arPrices)) return [];
        // if ($arCondition) {
        //     $arPrices = self::getPricesByFilter($arPrices, $arCondition);
        ...
        $arResultPrice = [];
        foreach ($arPrices as $arPrice) {
            if ($arPrice['PRICE'] < $maxPrice) {
                $maxPrice = $arPrice['PRICE'];
                $arResultPrice = $arPrice;
            }
        }
        return $arResultPrice;
    ...
    public static function getPriceFromOffersExt(array &$offers, array $arOptions)
    {
        ...
        if (!empty($offers) && is_array($offers)) {
            ...
            foreach ($offers as $oneOffer) {
                if (!$oneOffer["MIN_PRICE"] && $oneOffer["ITEM_PRICES"]) {
                    $oneOffer["MIN_PRICE"] = self::getCatalogPriceNew($oneOffer);
                }
    ...
    public static function show(array $arOptions = [])
    {
        ...
        //catalog
        if (self::$catalogInclude) {
            $arConfig['PRICES'] = !empty($arItem['PRICES']) ? self::getCatalogPrice($arItem['PRICES']) : self::getCatalogPriceNew($arItem);
            //set old price from price types
            $arOldPrice = !empty($arItem['PRICES']) ? self::getOldPriceFromTypes($arConfig['PRICES'], $arItem['PRICES']) : self::getOldPriceFromTypesNew($arConfig['PRICES'], $arItem);
                ...
                <div class="price <?=$arConfig['PRICE_BLOCK_CLASS'];?>">
                    ...
                    <div class="price__new">
                        <span class="price__new-val font_<?=$arConfig['PRICE_FONT'];?>">
                            ...
                            <?if ($arConfig['PRICES'] && self::$catalogInclude) :?>
    ...
    public static function getOldPriceFromTypesNew($arCurPrice, $arItem)
    {
        $arOldPrice = $arDiscountPrices = [];
        $arAllPrices = $arItem["ITEM_ALL_PRICES"][0]["PRICES"] ?? [];
        ...
        if ($arRegion
            && !empty($arRegion['PROPERTY_OLD_REGION_PRICES_VALUE'])
            && is_array($arRegion['PROPERTY_OLD_REGION_PRICES_VALUE'])
            && reset($arRegion['PROPERTY_OLD_REGION_PRICES_VALUE']) !== 'component'
        ) {
            $oldRegionPrices = $arRegion['PROPERTY_OLD_REGION_PRICES_VALUE'];
            $arDiscountPrices = array_filter($arAllPrices, fn($arValue) => in_array($arValue, $oldRegionPrices), ARRAY_FILTER_USE_KEY);
        } else {
            $sDiscountPrices = \Bitrix\Main\Config\Option::get(Solution::moduleID, 'DISCOUNT_PRICE', '');
            //set old price from price types
            if($arAllPrices){
                $arPricesCode = explode(',', $sDiscountPrices);
                $arPricesIds = static::getPriceTypeIdByCode($arPricesCode);
                $arDiscountPrices = array_intersect_key($arAllPrices, array_flip($arPricesIds));
            }
        }
        
        if( !empty($arDiscountPrices) ){
            $minOldPrice = self::getMinPriceNew($arDiscountPrices);
            
            if($minOldPrice['PRICE'] - $arCurPrice['DISCOUNT_VALUE'] > 0){
                $percent = round((($minOldPrice['PRICE'] - $arCurPrice['DISCOUNT_VALUE'])/$minOldPrice['PRICE'])*100, 0);
                $arOldPrice = $minOldPrice;
                $arOldPrice['VALUE'] = $arOldPrice['PRICE'];
                $arOldPrice['PRINT_VALUE'] = $arOldPrice['PRINT_PRICE'];
                $arOldPrice['DISCOUNT_DIFF_PERCENT'] = $percent;
            }                        
        }
    ...
    public static function getCatalogPriceNew(array $arItem = []){
        $arPrice = [];
        $arReplaceConfig = [
            // 'PRICE_CURRENCY' => "CURRENCY",
            'VALUE' => "BASE_PRICE",
            'PRINT_VALUE' => "PRINT_BASE_PRICE",
            'DISCOUNT_VALUE' => "PRICE",
            'DISCOUNT_DIFF_PERCENT' => "PERCENT",
            'PRINT_DISCOUNT_VALUE' => "PRINT_PRICE",

        ];
        if(!empty($arItem['ITEM_PRICES'][$arItem['ITEM_PRICE_SELECTED']])){
            $arPrice = $arItem['ITEM_PRICES'][$arItem['ITEM_PRICE_SELECTED']];

            foreach($arReplaceConfig as $oldKey => $newKey){
                $arPrice[$oldKey] = $arPrice[$newKey];
            }
        }
    ...
    public static function getPriceTypeIdByCode(array $priceType = []){
        $arPriceTypeIDs = [];

        $arAllPriceTypes = static::getAllPriceTypes();
        $arPriceTypeIDs = array_intersect($arAllPriceTypes, $priceType);

        return array_keys($arPriceTypeIDs);
    }

    public static function getAllPriceTypes(){
        static $arPriceTypes;

        if(!isset($arPriceTypes)){
            $arPriceTypes = [];
            $dbPriceType = \CCatalogGroup::GetList(
                [],
                [],
