...
<? if (isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower(
    ...
    ) == "xmlhttprequest" && isset($_GET["ajax_get"]) && $_GET["ajax_get"] == "Y" || (isset($_GET["ajax_basket"]) && $_GET["ajax_basket"] == "Y") || isset($_GET["control_ajax"])) {
    $isAjax = "Y";
} ?>
<? if (isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower(
        $_SERVER['HTTP_X_REQUESTED_WITH']
    ) == "xmlhttprequest" && isset($_GET["ajax_get_filter"]) && $_GET["ajax_get_filter"] == "Y" && !isset($_GET["control_ajax"])) {
    $isAjaxFilter = "Y";
} ?>
<?
    ...
    if ($arSKU['IBLOCK_ID']) {
        ...
        if ($arSkuIblock = $dbRes->Fetch()) {
            ...
            if (!$arSearchPageFilter['arrFILTER_iblock_' . $arSkuIblock['IBLOCK_TYPE_ID']]) {
                $arSearchPageFilter['arrFILTER_iblock_' . $arSkuIblock['IBLOCK_TYPE_ID']] = [];
            }
...
<div class="main-wrapper flexbox flexbox--direction-row">
    <div class="section-content-wrapper <? /*=($bShowLeftBlock ? 'with-leftblock' : '');*/ ?> flex-1">
        ...
        if (is_array($arElements) && !empty($arElements)) {
            if ($arSKU) {
                foreach ($arElements as $key => $value) {
                    $arTmp = CIBlockElement::GetProperty(
                        $arSKU['IBLOCK_ID'],
                        $value,
                        ["sort" => "asc"],
            ...
            if ($arRegion) {
                ...
                $GLOBALS[$arParams["FILTER_NAME"]] = TSolution::makeElementFilterInRegion(
                    $GLOBALS[$arParams["FILTER_NAME"]]
                );
            }
            ...
            <? $bContolAjax = (isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower(
                    $_SERVER['HTTP_X_REQUESTED_WITH']
                ) == "xmlhttprequest" && isset($_GET["control_ajax"]) && $_GET["control_ajax"] == "Y"); ?>
        ...
        <?php
            ...
            if (!strlen($searchQuery)) {
                echo '<div class="font-24">' . GetMessage("CT_BCSE_EMPTY_QUERY") . "</div>";
            } else {
                echo '<div class="font-24 text-center">' . GetMessage("CT_BCSE_NOT_FOUND") . "</div>";
            }
