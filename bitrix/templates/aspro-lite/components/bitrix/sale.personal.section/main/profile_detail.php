...
if($arUserPropValue)
{
	...
	if($arPhoneProp)
	{
		if($arUserPropValue[$arPhoneProp['ID']])
		{
			...
			{
				$mask = \Bitrix\Main\Config\Option::get('aspro.lite', 'PHONE_MASK', '+7 (999) 999-99-99');
			...
			<script>
				...
				{
					$('input[name=ORDER_PROP_<?=$arPhoneProp['ID']?>').inputmask('mask', {'mask': arAsproOptions['THEME']['PHONE_MASK']});
				})
			</script>
		<?}
	}
