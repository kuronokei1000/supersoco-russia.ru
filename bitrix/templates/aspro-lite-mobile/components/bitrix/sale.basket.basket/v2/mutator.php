...
foreach ($this->basketItems as $row) {
	...
	if ($row['DELAY'] !== 'Y') {
		...
		$arElement = CLiteCache::CIBLockElement_GetList(array('CACHE' => array("MULTI" => "N", "TAG" => CLiteCache::GetIBlockCacheTag($catalogIblockId))), $arElementFilter, false, false, array("ID", "IBLOCK_ID", "PROPERTY_SERVICES"));
