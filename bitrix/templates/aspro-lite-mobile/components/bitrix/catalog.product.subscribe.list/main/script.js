...
window.JCCatalogProductSubscribeList.prototype.Init = function()
{
	...
	if (0 === this.errorCode)
	{
		...
		var height = 0;
		var rect = this.obProduct.getBoundingClientRect();
		...
		BX.ajax({
			...
			onsuccess: BX.delegate(function (result) {
				if(result.success)
				{
					...
					let bDefaultNotice = false;

					if (typeof JNoticeSurface === 'function') {
						const surface = JNoticeSurface.get();
						const noticeFunc = "onAdd2Subscribe";
						const $node = this.deleteSubscribeBtn.closest('.js-popup-block');

						if (typeof surface[noticeFunc] === 'function' && $node) {
							surface[noticeFunc]([$node], false);
