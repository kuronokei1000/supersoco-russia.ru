						...
						<div class="line-block__item flex-1">
							<div class="brand__list">
								...
								<?foreach ($arBrands['ITEMS'] as $key => $arItem):?>
									<?if ($key > ($limit - 2)) break // only 16 elements?>
									<div class="brand__list-item">
										<a href="<?=$arItem['DETAIL_PAGE_URL'];?>" class="dark_link"><?=$arItem['NAME'];?></a>
									</div>
								<?endforeach;?>
								<?if (count($arBrands['ITEMS']) > $limit-1):?>
									<div class="brand__list-item">
										<span class="colored-link pointer filter-link" data-letter="<?=$arBrands['PREFIX'].$arBrands['CODE'];?>"><?=GetMessage('ALL_BRANDS_ON_LETTER', ['#LETTER#' => $letter]);?></span>
									</div>
								<?endif;?>
							</div>
						</div>
